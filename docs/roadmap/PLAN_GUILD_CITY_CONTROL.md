...
> **Numerotation (collision avec la roadmap globale) :** les numeros de section de **ce** document (52, 64-82, 67-69, etc.) sont des **jalons internes** au plan « controle de cite ». Ils ne correspondent **pas** aux taches du meme numero dans les fichiers `ROADMAP_TODO_VAGUE_*.md` (ex. tache globale **64** = equipement tier 3 ; **67-68** = cartes Foret / Mines). Seuls les prerequis explicites du type « ← 38 », « ← 48 » renvoient aux IDs de la roadmap globale. Le jalon **52** ici prolonge la tache globale **52** (guildes fondation).
...
**19 jalons internes** (numerotes 52, 64-82 dans ce document uniquement) organises en 7 pistes.
Depend des prerequis roadmap globale : **38** (amis), **48** (hub), **52** (guildes fondation).
