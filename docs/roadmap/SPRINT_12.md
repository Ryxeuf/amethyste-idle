...
> Prerequis : ∅
> Avancement : sous-phases 1 (selecteur de langue securise), 2a (parite UI FR/EN atteinte), 3a (infrastructure multilingue des noms d'items), 3b (cablage du filter Twig `localized_name` dans les templates shop/inventaire/bestiaire) et 3d (infrastructure multilingue des descriptions d'items + filter `localized_description`) livrees.
...
- [~] Contenu de jeu multilingue (noms items, descriptions sorts) — sous-phase 3a livree 2026-04-21
  ...
  - [x] **3d — Infrastructure multilingue pour les descriptions d'items** (2026-04-21) : colonne JSON `description_translations` ajoutee sur `game_items` (migration `Version20260421ItemDescriptionTranslations`, idempotente). Entite `Item` etendue avec `getLocalizedDescription(?string $locale): string` (fallback gracieux sur `$this->description` si locale nulle/vide, colonne nulle, locale absente ou valeur blanche), `getDescriptionTranslations(): array` (defaut `[]`) et `setDescriptionTranslations(?array $translations): Item` avec normalisation (cles/valeurs vides filtrees, stockage compact en `null` si aucune entree valide). Twig extension `ItemLocalizationExtension` etendue avec le filter `localized_description` (meme pattern que `localized_name` : lecture de la locale courante via `RequestStack`, fallback `''` si Item null). Tests `ItemLocalizationTest` etendus de 7 cas et `ItemLocalizationExtensionTest` etendu de 3 cas. Aucun template ni controller modifie : les sous-phases 3d.b (cablage) / 3d.c (fixtures EN) cableront progressivement.
