...
> Derniere mise a jour : 2026-04-28 (135 sous-phase 2e — i18n du panneau Stimulus d'inventaire `inventory_controller.js` : 17 nouvelles cles `game.inventory.{compare,rarity,element}.*` FR/EN, reuse des 12 cles existantes `game.inventory.slot.*`, helper `_label`, labels passes via `stimulus_controller(...)`)
