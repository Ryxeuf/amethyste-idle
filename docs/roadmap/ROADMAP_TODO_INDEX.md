...
> Derniere mise a jour : 2026-05-03 (130 sous-phase 6b — badge "Possédée" sur le catalogue de montures `/game/mounts` : `MountController` injecte `PlayerHelper` + `PlayerMountRepository`, nouveau `PlayerMountRepository::findOwnedMountIds(Player): list<int>` base sur `IDENTITY(pm.mount)`, badge emeraude "Possédée" + bordure de carte distincte pour les montures deja acquises, 2 cles FR/EN ajoutees `game.mount.owned`)
