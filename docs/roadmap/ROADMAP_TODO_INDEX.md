...
> Derniere mise a jour : 2026-05-04 (135 sous-phase 3d — cablage `localized_description` sur templates de production : reader `description_translations` ajoute dans `ItemFixtures`, 7 traductions EN de demo (potion/canne/pioche/buche + 3 materia debut de jeu), 3 templates Twig sur `{{ item|localized_description }}` (auction/catalog/shop), 3 controllers sur `getLocalizedDescription($locale)` (loot/inventory items/inventory materia). Diff ~28 lignes, ferme le gap d'integration de l'infrastructure deja livree par `Version20260421ItemDescriptionTranslations`)
...
| **Sprint 11** | Monde vivant | 6 | Basse | En cours (130 sous-phases 1 + 2a + 2b.shop + 2b.quest + 5 + 6 + 6b, 131 sous-phases 2a + 2b + 3 + 4, 132 sous-phases 1 + 2a + 2b + 3 + 4a + 4b.1 + 4b.1b + 4b.1c + 4b.2, 133 sous-phases 1 + 1b) |
