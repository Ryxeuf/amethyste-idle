...
> Derniere mise a jour : 2026-04-28 (135 sous-phase 3e.x — i18n complet de la page de groupe `/game/party` : 31 cles uniques `game.party.*` FR/EN couvrant invitations, vue du groupe, controles leader/membre, etat vide + 9 chaines JS injectees via une constante `PARTY_LABELS` au lieu des strings FR hardcodees dans `confirm()`/`showPartyMessage()`)
