...
| 20 | **ONB-05 → 20** | Le tunnel, la boucle du jeu et le coach ([PLAN_ONBOARDING.md](PLAN_ONBOARDING.md), Pistes B a E) | A faire **avec NAR-20 et ZON-39** : le tunnel neuf, le foyer d'attache et les PNJ du Fanal touchent les memes textes. Deux points d'attention : **ONB-08** (le parchemin ouvre un arbre) **precise GAME_DOMAINS §1** — a relire avec DOM ; **ONB-10** est une exigence de **donnees de zone** a instruire avec PLAN_ZONES (le Fanal n'expose qu'une seule recolte, donc « au choix » y est aujourd'hui impossible) |
