...
> Derniere mise a jour : 2026-05-04 (130 sous-phase 2b.quest — recompense de quete pour les montures via `MountQuestRewardGranter` : nouveau service idempotent qui resout le slug, delegue a `MountAcquisitionService::grantMount` avec `SOURCE_QUEST` et silently no-op si deja possedee. `QuestController::applyRewards` accepte une cle `mounts: list<string>` dans `Quest::rewards` JSON, couvre simultanement les quetes principales et quotidiennes via la methode commune)
