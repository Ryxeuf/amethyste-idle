...
> Derniere mise a jour : 2026-08-02 (**point global doc↔code** : ONB 17/20 — restent ONB-02/04, 07b, 20b-b ; MAT 8/8 ✅ ; BES 6/6 ✅ ; OBJ 4/8 ; RET 10/10 ✅ avec la vague 4 ; WIK livre ✅ ; DON ouvert, DON-01a livre ; restes ZON-26b + ZON-41 livres ; **front actuel : DON, puis ARC/MET a ouvrir**) ; 2026-07-29 (**ouverture du plan Onboarding ONB-01→15** — le jeu n'avait aucune porte d'entree : cf. [PLAN_ONBOARDING.md](PLAN_ONBOARDING.md)) ; 2026-07-29 (**point global doc↔code** : DOM 8/8 complet, ZON-30→36 livres, FAC ouvert 1/10, FOY 17/17 + vague 2 FOY-18→21 ouverte, RET 7/7) ; 2026-07-26 (**tache 130** montures, **Sprint 13 clos** par ZON-26b-a, **Sprints 14-15 termines**, **housing complet**) ; 2026-07-25 (**point post-pivot** : campagne ZON close, menage des sprints 11-12, ouverture des Sprints 13-14)
...
| Plan Onboarding (ONB-01→20) | 17/20 | Quasi complet (2026-07-31) — restent **ONB-02/04 (mailer, dernier bloquant dur)**, ONB-07b, ONB-20b-b |
...
| Plan Objets (OBJ-01→08) | 4/8 | En cours — OBJ-01→04 livres (2026-08-02) ; restent OBJ-05→08 (outils, champignon, tests) |
| **Plan Donjons (DON-01→06)** | **DON-01a livre** | **En cours (2026-08-02)** — front actuel ; DON-02 croise ARC (sequencement **tranche le 2026-08-02 : DON d'abord, pont assume** — MAT livre, DON-02 branche les vraies actions de sort ; la melee garde l'attaque de base jusqu'a ARC-02, dette documentee) |
...
| 15 | **ONB-01 → 04** ◐ | Le compte peut naitre ([PLAN_ONBOARDING.md](PLAN_ONBOARDING.md), Piste A) | **ONB-01 et ONB-03 livres le 2026-07-29** (`/register` existe, `isBanned` est lu, la connexion mene la ou le joueur en est). Restent **ONB-02** (mailer + mot de passe oublie — infrastructure **decidee le 2026-08-02** : Brevo, envoi depuis `no-reply@amethyste.best`, code livre d'abord avec `MAILER_DSN` en env, branchement prod quand compte + DNS sont prets) et **ONB-04** (verification differee, ← ONB-02). **Perdre son mot de passe fait toujours perdre son compte** — dernier bloquant dur d'une ouverture a de vrais joueurs |
...
| 16 quinquies | **DON-01 → 05** ⚙ | Les donjons ([PLAN_DUNGEONS.md](PLAN_DUNGEONS.md)) | **En cours — DON-01a livre le 2026-08-02** : les deux donjons solo rejoignent le graphe des zones (`maxPlayers: 1`, lancement seul), le seuil d'XP n'a plus qu'une maison. Reste **DON-01b** (suppression du chemin mort) puis **DON-02→06**. **DON-02 croise ARC** (« le geste d'arme devient une materia ») : sequencement **tranche le 2026-08-02 : DON d'abord, pont assume** — MAT livre, DON-02 branche les vraies actions de sort ; la melee garde l'attaque de base jusqu'a ARC-02, dette documentee. **DON-03 suppose BES-01 ✅** et **DON-04 converge avec MAT-06 ✅** — les deux prerequis sont livres. Le contenu neuf se limite a **un donjon T3** |
...
- [Donjons (DON-01 a DON-06)](PLAN_DUNGEONS.md) ⚙ — **en cours, DON-01a livre le 2026-08-02** : les deux donjons solo (qui reposaient sur la carte supprimee par ZON-21) rejoignent le graphe des zones en `maxPlayers: 1` et se lancent seuls, le seuil d'XP n'a plus qu'une maison. Reste **DON-01b** (suppression du chemin mort `originMap`/`originCoordinates`) puis **DON-02→06** : la rencontre de groupe est encore un sac de PV sans `Monster`, le degat d'un joueur est son `hit`, il n'y a **aucune riposte donc aucun echec possible**, `currentStep` est inerte et le butin est en gils seuls quand `lootPreview` promet de l'equipement. Un seul modele (le donjon de zone), trois etapes puisant dans la faune du palier (**BES ✅**), **DON-04 est le canal m4-m5 qu'attend MAT-06 ✅**, et un donjon par palier T1-T4 — la Cite ensevelie etant deja declaree « T4 — donjon » par GAME_ZONES §2, le cout net est **un seul donjon a ecrire**. **DON-02 croise ARC** (le geste d'arme comme materia) : sequencement **tranche le 2026-08-02 : DON d'abord, pont assume** — MAT livre, DON-02 branche les vraies actions de sort ; la melee garde l'attaque de base jusqu'a ARC-02, dette documentee. Decline [docs/GAME_DUNGEONS.md](../GAME_DUNGEONS.md)
...
- [Objets et ressources (OBJ-01 a OBJ-08)](PLAN_ITEMS.md) — **4/8, OBJ-01→04 livres le 2026-08-02** : taxonomie alignee sur les 5 constantes (60 matieres rendues a l'onglet Materiaux), menage des doublons et du hors-perimetre (verse a `docs/EXTENSION_RESERVE.md`), equipement redevenu neutre (56 pieces elementaires fusionnees en 14, grille 3 paliers × 7 formes verrouillee), emplacements de materia progressifs 1/2/3 et lanceurs types `Spell` (**le versant Technique est une dette declaree sur ARC** : aucune materia de technique n'existe encore). **Restent OBJ-05→08** : l'outil de recolte (la recolte en exige un, le palier module le rendement), les paliers d'outil et les 3 metiers manquants, le champignon et l'equilibre des lignes, les tests du plan. Decline [docs/GAME_ITEMS.md](../GAME_ITEMS.md)
