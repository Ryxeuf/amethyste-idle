...
> Derniere mise a jour : 2026-05-03 (135 sous-phase 3e.z — i18n complet des pages Donjons `/game/dungeon/list` + `/game/dungeon/show` : 25 cles uniques `game.dungeon.*` FR/EN couvrant titre, banner run en cours, liste, historique, niveau/joueurs, infos, requirements, loot preview, difficulte (`normal/heroic/mythic` traduits via cle dynamique `('game.dungeon.difficulty.' ~ diff.value)|trans`), badges Cooldown/Indisponible, bouton Entrer)
...
| **Sprint 11** | Monde vivant | 6 | Basse | En cours (130 sous-phases 1 + 5 + 6, 131 sous-phases 2a + 2b + 3 + 4, 132 sous-phases 1 + 2a + 2b + 3 + 4a + 4b.1 + 4b.2, 133 sous-phases 1 + 1b) |
