...
- [docs/GAME_ONBOARDING.md](docs/GAME_ONBOARDING.md) — **Cadrage de l'entree dans le jeu** (source de verite du compte, du personnage et des dix premieres minutes) : l'inscription avec **verification d'e-mail differee** (elle ne barre pas le jeu, elle barre le marche, les guildes, le chat et la livraison aux foyers — donc le multicomptage), le **tunnel unique en 4 pas** (compte → nom → peuple → visage) porte par une seule fiction, **aucune decision de build dans le tunnel** — le peuple ne determine ni le metier, ni l'element, ni la destination, ni les arbres visibles ; le **foyer d'attache se gagne, il ne se choisit pas** (c'est la zone ou le joueur a travaille pendant l'acte I — **amendement a GAME_WORLD §13.1**, qui le derivait de la race) ; les **32 arbres en deux couches** (la carte du savoir, toujours complete, et « mes arbres », ouverts un a un par les gestes — application directe de la doctrine des trois couches de GAME_DOMAINS, jamais un filtre par peuple) ; les **neuf gestes de l'acte I** ou le tutoriel impose le verbe et le joueur choisit le complement (quelle recolte, **quel element de materia** en 5e, quelle destination), voyage en 8e parce qu'il fait attendre et **expedition en cloture** — comment quitter le jeu en le laissant travailler ; le **coach par ecran** (un ecran jamais ouvert se presente une fois, dit ce que ca coute, et ne parle jamais d'un systeme inutilisable) et les 12 dettes constatees dans le code. Premier ecran apres creation : **la zone, jamais le hub**. Decline en ONB-01→17
