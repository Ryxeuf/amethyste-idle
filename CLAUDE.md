...
- [docs/GAME_WORLD.md](docs/GAME_WORLD.md) — **Socle de monde** (proposition) : geographie, biomes, politique, trame globale. Postulat unique (le temps vecu se depose en amethystite) d'ou decoulent les biomes (un biome = une facon dont le temps s'est depose), les regions (Sanctuaire, Plaines, Terres Sauvages, l'Ambre, le Silence, le Blanc), la politique (la Concorde, le Serment = raison interne du « pas de PvP », axe doctrinal Extraire/Preserver, faction **la Fonderie** a creer) et le scenario (acte d'intro / marees saisonnieres / meta-arc du Reflux). A lire **avant** d'ajouter une zone, une faction ou un theme de saison
