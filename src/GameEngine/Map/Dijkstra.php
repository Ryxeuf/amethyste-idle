...
class Dijkstra
{
    ...
    // Ability to check if a coordinate is accessible
    protected int $abilityMask;
     ...
     * from the queue.
     */
    private function getNeighbors(string $origin): array
    {
        ...
        foreach ($allNeighbors as $neighborId => $distance) {
            if (($distance & $this->abilityMask) === $distance && $this->queue->contains($neighborId)) {
