<?php

namespace App\DataFixtures;

use App\Entity\Game\Spell;
use App\Enum\CombatRegister;
use App\Enum\Element;
use App\Enum\SpellIntent;
use App\Enum\SpellScope;
use Doctrine\Bundle\FixturesBundle\Fixture;
use Doctrine\Persistence\ObjectManager;

class SpellFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        $spellsData = $this->getSpellsData();

        // Création des sorts pour chaque élément
        foreach ($spellsData as $key => $data) {
            $spell = new Spell();
            $spell->setSlug($data['slug']);
            $spell->setDamage($data['damage']);
            $spell->setElement($data['element']);
            $spell->setHeal($data['heal']);
            $spell->setName($data['name']);
            $spell->setDescription($data['description']);
            $spell->setHit($data['hit'] ?? 90);
            $spell->setCritical($data['critical'] ?? 5);
            $spell->setEnergyCost($data['energyCost'] ?? 0);
            $spell->setCooldown($data['cooldown'] ?? null);
            // ARC-04b : la ressource du registre distance.
            $spell->setAmmoCost($data['ammoCost'] ?? 0);
            $spell->setStatusEffectSlug($data['statusEffectSlug'] ?? null);
            $spell->setAoeTargets($data['aoeTargets'] ?? 1);
            $spell->setLevel($data['level'] ?? 1);
            $spell->setValueType($data['valueType'] ?? 'fixed');
            // ARC-02 : le geste declare son registre, et la materia en herite.
            // Tout ce qui est livre ici est un **sort** ; les techniques
            // (melee, distance) arrivent avec les arbres qui les ouvrent.
            $spell->setRegister($data['register'] ?? CombatRegister::Spell);
            // ARC-11a — la decision d'auteur quand il y en a une. Les colonnes
            // restent nullables : sans declaration, `SpellIntentDeriver`
            // repond, et la donnee suffit dans 253 cas sur 253.
            $spell->setIntent($data['intent'] ?? null);
            $spell->setScope($data['scope'] ?? null);
            $spell->setCreatedAt(new \DateTime());
            $spell->setUpdatedAt(new \DateTime());

            $manager->persist($spell);
            $this->addReference($key, $spell);
        }

        $manager->flush();
    }

    /**
     * Retourne les données des sorts.
     */
    private function getSpellsData(): array
    {
        return [
            // Sorts de feu
            'fire_ball' => [
                'slug' => 'fire-ball',
                'damage' => 2,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Boule de feu',
                'description' => 'Une boule de feu pour tout cramer',
                'hit' => 90,
                'energyCost' => 5,
                'statusEffectSlug' => 'burn',
                'critical' => 8,
                'level' => 1,
            ],
            'flame' => [
                'slug' => 'flame',
                'damage' => 1,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Flammèche',
                'description' => 'Un sort de flammèche',
                'hit' => 90,
                'level' => 1,
                'statusEffectSlug' => 'burn',
            ],
            'flamer' => [
                'slug' => 'flamer',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Feu',
                'description' => 'Un sort de feu',
                'hit' => 90,
                'level' => 2,
            ],
            'flame_rain' => [
                'slug' => 'flame-rain',
                'damage' => 5,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Pluie de flammes',
                'description' => 'Une pluie de flammes qui carbonise tout sur une large zone',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'statusEffectSlug' => 'burn',
                'critical' => 10,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'inferno' => [
                'slug' => 'inferno',
                'damage' => 7,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Inferno',
                'description' => 'Un brasier dévastateur qui consume tout sur son passage',
                'hit' => 85,
                'energyCost' => 20,
                'cooldown' => 3,
                'statusEffectSlug' => 'burn',
                'critical' => 12,
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // ARC-07a — l'accord **non-degat** du Pyromancien (§ 5.1, loi 2 :
            // *le tour ou frapper n'est pas la reponse*). Le § 9.1 le decrit
            // comme « le geste de temporisation : il ne tue pas, il retient »,
            // et l'intention doit donc etre **declaree** : la derivation lit le
            // degat d'abord (ARC-11a), et rendrait `degat` pour un geste dont
            // ce n'est pas le propos. C'est exactement l'usage que la colonne
            // nullable de `Spell::intent` attendait.
            'fire_wall' => [
                'slug' => 'fire-wall',
                'damage' => 4,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Mur de feu',
                'description' => 'Crée un mur de flammes infranchissable',
                'hit' => 95,
                'level' => 2,
                'aoeTargets' => 2,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Targets,
            ],
            // ARC-07a — l'accord de la branche **Braise** (§ 9.1). *Il ne
            // frappe pas, il reste* : la Brulure est ce qui dure, et `grip` —
            // la teinte de cette seule branche — l'allonge. Sans un geste par
            // branche, la fourche serait une decoration : deux branches qui ne
            // different que par leurs passifs produisent le meme combat, au
            // tour pres (§ 9 bis).
            'brasier' => [
                'slug' => 'brasier',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Brasier',
                'description' => 'Un foyer qui ne s\'eteint pas : ce qu\'il touche continue de brûler',
                'hit' => 90,
                'energyCost' => 12,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 2,
                'critical' => 5,
                'level' => 3,
            ],
            'fire_nova' => [
                'slug' => 'fire-nova',
                'damage' => 6,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Nova de feu',
                'description' => 'Une explosion de feu qui se propage dans toutes les directions',
                'hit' => 85,
                'level' => 3,
            ],
            'phoenix_flame' => [
                'slug' => 'phoenix-flame',
                'damage' => 5,
                'element' => Element::Fire,
                'heal' => 2,
                'name' => 'Flamme du phénix',
                'description' => 'Des flammes régénératrices qui brûlent et soignent à la fois',
                'hit' => 90,
                'level' => 3,
            ],
            'meteor_strike' => [
                'slug' => 'meteor-strike',
                'damage' => 8,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Frappe météorique',
                'description' => 'Invoque un météore enflammé qui s\'écrase sur l\'ennemi',
                'hit' => 75,
                'level' => 4,
            ],
            'burning_touch' => [
                'slug' => 'burning-touch',
                'damage' => 2,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Toucher brûlant',
                'description' => 'Un simple contact qui inflige des brûlures',
                'hit' => 100,
                'level' => 1,
            ],
            'heat_wave' => [
                'slug' => 'heat-wave',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Vague de chaleur',
                'description' => 'Une onde de chaleur qui affaiblit les ennemis',
                'hit' => 95,
                'level' => 2,
            ],
            'dragon_breath' => [
                'slug' => 'dragon-breath',
                'damage' => 6,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Souffle du dragon',
                'description' => 'Un souffle de feu devastateur',
                'hit' => 85,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 0,
                'critical' => 10,
                'level' => 4,
            ],
            'volcanic_eruption' => [
                'slug' => 'volcanic-eruption',
                'damage' => 10,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Eruption volcanique',
                'description' => 'Dechaine la puissance d\'un volcan sur les ennemis',
                'hit' => 70,
                'energyCost' => 30,
                'cooldown' => 5,
                'statusEffectSlug' => 'burn',
                'critical' => 15,
                'aoeTargets' => 0,
                'level' => 5,
            ],
            'ember_shield' => [
                'slug' => 'ember-shield',
                // ARC-08c — **un bouclier qui brulait son porteur.** Le geste
                // se posait `sur soi` et appliquait `burn` : la description
                // dit « brule les attaquants », mais le statut suit la portee,
                // et la portee est le lanceur. Il portait aussi un degat de 1,
                // qui frappait donc celui qu'il protege. Les deux partent —
                // *ce n'etait pas un choix d'equilibrage, c'etait un effet a
                // l'envers* — et le soin reste, avec le `shield` que sa
                // fonction reclame.
                'damage' => null,
                'element' => Element::Fire,
                'heal' => 3,
                'name' => 'Bouclier d\'étincelles',
                'description' => 'Un voile de braises qui absorbe ce qui vient',
                'hit' => 100,
                'energyCost' => 0,
                'ammoCost' => 2,
                'level' => 2,
                'statusEffectSlug' => 'shield',
                'register' => CombatRegister::Ranged,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::SelfOnly,
            ],
            'fire_whip' => [
                'slug' => 'fire-whip',
                'damage' => 4,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Fouet de feu',
                'description' => 'Un fouet enflammé qui frappe à distance',
                'hit' => 90,
                'level' => 2,
            ],
            'combustion' => [
                'slug' => 'combustion',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Combustion',
                'description' => 'Enflamme instantanément la cible',
                'hit' => 95,
                'level' => 2,
            ],

            // Sorts de vie
            'life_heal' => [
                'slug' => 'life-heal',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 5,
                'name' => 'Soin mineur',
                'description' => 'Un soin mineur qui fait du bien',
                'hit' => 100,
                'energyCost' => 5,
                'level' => 1,
            ],
            'rejuvenation' => [
                'slug' => 'rejuvenation',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 8,
                'name' => 'Regeneration',
                'description' => 'Un sort de regeneration qui restaure progressivement la sante',
                'hit' => 100,
                'energyCost' => 10,
                'statusEffectSlug' => 'regeneration',
                'level' => 2,
            ],
            'divine_blessing' => [
                'slug' => 'divine-blessing',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 12,
                'name' => 'Benediction divine',
                'description' => 'Une puissante benediction qui soigne les blessures graves',
                'hit' => 100,
                'energyCost' => 18,
                'cooldown' => 3,
                'statusEffectSlug' => 'regeneration-strong',
                'level' => 4,
            ],
            'healing_wave' => [
                'slug' => 'healing-wave',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 7,
                'name' => 'Vague de guérison',
                'description' => 'Une vague d\'énergie curative qui soigne les blessures',
                'hit' => 100,
                'level' => 2,
            ],
            'life_transfer' => [
                'slug' => 'life-transfer',
                'damage' => 2,
                'element' => Element::Light,
                'heal' => 4,
                'name' => 'Transfert de vie',
                'description' => 'Transfère l\'énergie vitale de l\'ennemi vers soi',
                'hit' => 90,
                'level' => 2,
            ],
            'holy_light' => [
                'slug' => 'holy-light',
                'damage' => 3,
                'element' => Element::Light,
                'heal' => 3,
                'name' => 'Lumiere',
                'description' => 'Une lumiere divine qui soigne les allies et blesse les ennemis',
                'hit' => 95,
                'energyCost' => 6,
                'level' => 2,
                'statusEffectSlug' => 'revealed',
            ],
            'vitality_surge' => [
                'slug' => 'vitality-surge',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 10,
                'name' => 'Afflux de vitalité',
                'description' => 'Un puissant afflux d\'énergie vitale qui restaure la santé',
                'hit' => 100,
                'level' => 3,
            ],
            'life_shield' => [
                'slug' => 'life-shield',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 6,
                'name' => 'Bouclier de vie',
                'description' => 'Crée un bouclier protecteur qui absorbe les dégâts',
                'hit' => 100,
                'level' => 2,
            ],
            'angelic_touch' => [
                'slug' => 'angelic-touch',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 4,
                'name' => 'Toucher angélique',
                'description' => 'Un toucher qui apaise les blessures',
                'hit' => 100,
                'level' => 1,
            ],
            'purification' => [
                'slug' => 'purification',
                'damage' => 1,
                'element' => Element::Light,
                'heal' => 5,
                'name' => 'Purification',
                'description' => 'Purifie le corps et l\'esprit, eliminant les afflictions',
                'hit' => 100,
                'energyCost' => 10,
                'level' => 3,
            ],
            'celestial_blessing' => [
                'slug' => 'celestial-blessing',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 15,
                'name' => 'Bénédiction céleste',
                'description' => 'Une bénédiction des cieux qui restaure une grande quantité de santé',
                'hit' => 95,
                'level' => 5,
            ],
            'life_burst' => [
                'slug' => 'life-burst',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 9,
                'name' => 'Explosion de vie',
                'description' => 'Une explosion d\'énergie vitale qui soigne instantanément',
                'hit' => 100,
                'level' => 3,
            ],
            'divine_intervention' => [
                'slug' => 'divine-intervention',
                'damage' => 5,
                'element' => Element::Light,
                'heal' => 5,
                'name' => 'Jugement sacre',
                'description' => 'Un jugement divin qui soigne les allies et punit les ennemis',
                'hit' => 90,
                'energyCost' => 22,
                'cooldown' => 4,
                'critical' => 15,
                'level' => 5,
            ],
            'healing_touch' => [
                'slug' => 'healing-touch',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 3,
                'name' => 'Toucher guérisseur',
                'description' => 'Un simple toucher qui soigne les blessures légères',
                'hit' => 100,
                'level' => 1,
            ],
            'life_bloom' => [
                'slug' => 'life-bloom',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 6,
                'name' => 'Floraison de vie',
                'description' => 'Une énergie vitale qui se répand progressivement',
                'hit' => 100,
                'level' => 2,
            ],
            'sacred_light' => [
                'slug' => 'sacred-light',
                'damage' => 4,
                'element' => Element::Light,
                'heal' => 2,
                'name' => 'Lumière sacrée',
                'description' => 'Un rayon de lumière divine qui purifie et soigne',
                'hit' => 95,
                'level' => 2,
            ],

            // Sorts de mort
            'punishment' => [
                'slug' => 'punishment',
                'damage' => 1,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Châtiment',
                'description' => 'Un sort de châtiment',
                'hit' => 90,
                'level' => 1,
                'statusEffectSlug' => 'blinded',
            ],
            'soul_drain' => [
                'slug' => 'soul-drain',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => 1,
                'name' => 'Drain de vie',
                'description' => 'Aspire l\'energie vitale de la cible pour se soigner',
                'hit' => 85,
                'energyCost' => 6,
                'level' => 2,
                'statusEffectSlug' => 'blinded',
            ],
            'death_touch' => [
                'slug' => 'death-touch',
                'damage' => 4,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Toucher mortel',
                'description' => 'Un contact qui inflige une douleur intense',
                'hit' => 80,
                'level' => 2,
            ],
            'shadow_bolt' => [
                'slug' => 'shadow-bolt',
                'damage' => 5,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Éclair d\'ombre',
                'description' => 'Un projectile d\'énergie sombre qui frappe la cible',
                'hit' => 85,
                'level' => 3,
            ],
            'life_leech' => [
                'slug' => 'life-leech',
                'damage' => 4,
                'element' => Element::Dark,
                'heal' => 2,
                'name' => 'Sangsue vitale',
                'description' => 'Absorbe l\'énergie vitale de la cible',
                'hit' => 90,
                'level' => 2,
            ],
            'death_grip' => [
                'slug' => 'death-grip',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Emprise de la mort',
                'description' => 'Une main spectrale qui saisit et blesse la cible',
                'hit' => 95,
                'level' => 2,
            ],
            'shadow_wave' => [
                'slug' => 'shadow-wave',
                'damage' => 6,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Vague d\'ombre',
                'description' => 'Une vague d\'énergie sombre qui balaye la zone',
                'hit' => 80,
                'level' => 3,
            ],
            'necrotic_touch' => [
                'slug' => 'necrotic-touch',
                'damage' => 2,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Toucher nécrotique',
                'description' => 'Un toucher qui provoque la nécrose des tissus',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 0,
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            'dark_harvest' => [
                'slug' => 'dark-harvest',
                'damage' => 7,
                'element' => Element::Dark,
                'heal' => 3,
                'name' => 'Moisson sombre',
                'description' => 'Recolte l\'energie vitale des ennemis affaiblis',
                'hit' => 75,
                'energyCost' => 18,
                'cooldown' => 3,
                'statusEffectSlug' => 'poison-strong',
                'critical' => 12,
                'level' => 4,
            ],
            'soul_rip' => [
                'slug' => 'soul-rip',
                'damage' => 8,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Déchirure d\'âme',
                'description' => 'Déchire l\'âme de la cible, causant une douleur intense',
                'hit' => 70,
                'level' => 4,
            ],
            'death_nova' => [
                'slug' => 'death-nova',
                'damage' => 6,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Nova de mort',
                'description' => 'Une explosion d\'énergie mortelle qui se propage',
                'hit' => 80,
                'level' => 3,
            ],
            'shadow_mend' => [
                'slug' => 'shadow-mend',
                'damage' => 1,
                'element' => Element::Dark,
                'heal' => 5,
                'name' => 'Guérison des ombres',
                'description' => 'Utilise l\'énergie sombre pour soigner les blessures',
                'hit' => 95,
                'level' => 2,
            ],
            // ARC-08a — l'entrave sur la duree du Necromancien (§ 12.2). Son
            // intention est **declaree** pour la meme raison que le Mur de feu
            // (ARC-07a) : la derivation lit le degat d'abord, et rendrait
            // `degat` pour un geste dont le propos est de maudire. Le degat
            // reste ce qu'il etait — ce qui change, c'est ce que le geste
            // qualifie : `power` ne l'amplifie plus, `grip` l'allonge.
            'plague_strike' => [
                'slug' => 'plague-strike',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Malediction',
                'description' => 'Maudit la cible, empoisonnant son corps et son ame',
                'hit' => 90,
                'energyCost' => 10,
                'statusEffectSlug' => 'poison',
                'level' => 3,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Target,
            ],
            // ARC-08a — le premier geste hostile du jeu qui ne blesse pas.
            //
            // Les trente gestes a zero degat livres sont **tous** amicaux
            // (boucliers et regenerations) : le Voile est le premier a poser un
            // statut sur un adversaire sans lui retirer un point de vie, et
            // c'est la signature de la fonction controle — *ses trois premiers
            // tours ne tuent rien*.
            //
            // Il tient la loi de duree d'ARC-13a **par le second membre** :
            // Aveugle dure 2 tours, donc `durationIsLegal(2, false)` est vraie.
            // Une entrave d'un tour serait un nœud mort — le § 9 quinquies l'a
            // mesure — mais celle-ci n'en est pas une.
            'ash_veil' => [
                'slug' => 'ash-veil',
                'damage' => null,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Voile de cendre',
                'description' => 'Une cendre qui ne retombe pas : la cible ne voit plus ou elle frappe',
                'hit' => 90,
                'energyCost' => 15,
                'statusEffectSlug' => 'blinded',
                'level' => 2,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Target,
            ],
            // ARC-08a — l'accord de la branche **Linceul** (§ 12.3). *Rien ne me
            // touche parce que rien ne joue* : il etouffe, la ou la Veillee
            // depose. Le Silence est le seul statut d'entrave d'element
            // tenebres du catalogue, et c'est sa duree (3 tours) qui en fait
            // une entrave et non une reaction.
            'shroud' => [
                'slug' => 'shroud',
                'damage' => null,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Linceul',
                'description' => 'Un drap d\'ombre qui etouffe le geste avant qu\'il ne parte',
                'hit' => 85,
                'energyCost' => 25,
                'cooldown' => 3,
                'statusEffectSlug' => 'silence',
                'level' => 4,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Target,
            ],
            // ARC-08a — l'accord de la branche **Veillee** (§ 12.3).
            //
            // **Le canon en fait un familier, et le familier n'existe pas.** Le
            // § 13.3 le range parmi les *formes de geste* — un depot offensif
            // qui frappe sur les tours ou son invocateur n'est pas la —, et les
            // formes sont le chantier d'ARC-18 : `DepositLaw` ne depose
            // aujourd'hui que la portee `Group` et la protection.
            //
            // On ecrit donc ce que la mecanique livree sait tenir — une chose
            // qui continue de frapper apres le tour ou on l'a posee — et on
            // nomme ce qui manque, comme ARC-07b l'a fait pour la Dissipation.
            // La fiction est deja la ; c'est la loi du depot qui devra
            // l'accueillir.
            'bone_servant' => [
                'slug' => 'bone-servant',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Serviteur d\'ossements',
                'description' => 'Ce qui a servi sert encore : il frappe tant qu\'il tient debout',
                'hit' => 85,
                'energyCost' => 20,
                'statusEffectSlug' => 'poison-slow',
                'level' => 3,
            ],
            'death_coil' => [
                'slug' => 'death-coil',
                'damage' => 5,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Spirale de mort',
                'description' => 'Une spirale d\'énergie nécrotique qui frappe la cible',
                'hit' => 85,
                'level' => 3,
            ],
            'soul_siphon' => [
                'slug' => 'soul-siphon',
                'damage' => 4,
                'element' => Element::Dark,
                'heal' => 4,
                'name' => 'Siphon d\'âme',
                'description' => 'Siphonne l\'essence vitale de la cible',
                'hit' => 85,
                'level' => 3,
            ],
            'dark_ritual' => [
                'slug' => 'dark-ritual',
                'damage' => 2,
                'element' => Element::Dark,
                'heal' => 6,
                'name' => 'Rituel sombre',
                'description' => 'Un rituel qui sacrifie une partie de sa force pour se soigner',
                'hit' => 100,
                'level' => 3,
            ],

            // Sorts de terre
            'stone_throw' => [
                'slug' => 'stone-throw',
                'damage' => 1,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Jet de cailloux',
                'description' => 'Une volee de pierres qui pese sur qui la recoit',
                'hit' => 90,
                'energyCost' => 0,
                'cooldown' => 0,
                // ARC-08d : la marque de la terre passe sur l'accord d'entree
                // du Gardien. Elle etait portee par le Bouclier terreux, un
                // geste **partage** avec le Geomancien — donc de registre
                // `spell`, donc hors de portee d'un arbre de melee.
                'statusEffectSlug' => 'weighed-down',
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            'earthquake' => [
                'slug' => 'earthquake',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Tremblement de terre',
                'description' => 'Provoque un séisme qui déstabilise les ennemis',
                'hit' => 85,
                'level' => 2,
            ],
            'rock_armor' => [
                'slug' => 'rock-armor',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 3,
                'name' => 'Parade',
                'description' => 'Une parade rocheuse qui absorbe les degats',
                'hit' => 100,
                'energyCost' => 5,
                'statusEffectSlug' => 'shield',
                'level' => 1,
            ],
            'boulder_throw' => [
                'slug' => 'boulder-throw',
                'damage' => 6,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Lancer de rocher',
                'description' => 'Lance un énorme rocher sur l\'ennemi',
                'hit' => 80,
                'level' => 3,
            ],
            'earth_spike' => [
                'slug' => 'earth-spike',
                'damage' => 3,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Pic de terre',
                'description' => 'Un pic rocheux jaillit du sol et transperce l\'ennemi',
                'hit' => 90,
                'level' => 2,
            ],
            'stone_spikes' => [
                'slug' => 'stone-spikes',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Pics de pierre',
                'description' => 'Fait jaillir plusieurs pics de pierre acérés du sol',
                'hit' => 88,
                'level' => 2,
            ],
            'landslide' => [
                'slug' => 'landslide',
                'damage' => 5,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Glissement de terrain',
                'description' => 'Déclenche un glissement de terrain qui ensevelit les ennemis',
                'hit' => 85,
                'level' => 3,
            ],
            'stone_skin' => [
                'slug' => 'stone-skin',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 5,
                'name' => 'Bouclier magique',
                'description' => 'Un bouclier de pierre magique offrant une protection accrue',
                'hit' => 100,
                'energyCost' => 10,
                'statusEffectSlug' => 'shield-strong',
                'level' => 3,
            ],
            'earth_shield' => [
                'slug' => 'earth-shield',
                'damage' => 1,
                'element' => Element::Earth,
                'heal' => 4,
                'name' => 'Bouclier terreux',
                'description' => 'Un bouclier de terre qui protège et contre-attaque',
                'hit' => 100,
                'level' => 2,
                'statusEffectSlug' => 'weighed-down',
            ],
            'crystal_spear' => [
                'slug' => 'crystal-spear',
                'damage' => 7,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Lance de cristal',
                'description' => 'Une lance de cristal tranchante qui transperce les armures',
                'hit' => 85,
                'level' => 4,
            ],
            'quicksand' => [
                'slug' => 'quicksand',
                'damage' => 2,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Sables mouvants',
                'description' => 'Transforme le sol en sables mouvants qui engloutissent l\'ennemi',
                'hit' => 95,
                'level' => 1,
                'statusEffectSlug' => 'weighed-down',
            ],
            'stone_wall' => [
                'slug' => 'stone-wall',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 6,
                'name' => 'Mur de fer',
                'description' => 'Erige un mur de fer infranchissable',
                'hit' => 100,
                'energyCost' => 20,
                'cooldown' => 4,
                'statusEffectSlug' => 'shield-strong',
                'level' => 4,
            ],
            'earth_blessing' => [
                'slug' => 'earth-blessing',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 8,
                'name' => 'Bénédiction de la terre',
                'description' => 'Invoque la bénédiction de la terre pour soigner et protéger',
                'hit' => 100,
                'level' => 3,
            ],
            'mountain_strength' => [
                'slug' => 'mountain-strength',
                'damage' => 3,
                'element' => Element::Earth,
                'heal' => 3,
                'name' => 'Force de la montagne',
                'description' => 'Puise dans la force des montagnes pour renforcer le corps',
                'hit' => 95,
                'level' => 2,
            ],
            'crystal_growth' => [
                'slug' => 'crystal-growth',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Croissance cristalline',
                'description' => 'Des cristaux acérés poussent rapidement et blessent l\'ennemi',
                'hit' => 90,
                'level' => 2,
            ],
            'tectonic_shift' => [
                'slug' => 'tectonic-shift',
                'damage' => 8,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Déplacement tectonique',
                'description' => 'Provoque un violent déplacement des plaques tectoniques',
                'hit' => 75,
                'level' => 5,
            ],
            'petrification' => [
                'slug' => 'petrification',
                'damage' => 5,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Pétrification',
                'description' => 'Transforme partiellement l\'ennemi en pierre',
                'hit' => 80,
                'level' => 3,
            ],

            // Sorts de métal
            'sword_10' => [
                'slug' => 'sword-strike',
                'damage' => 1,
                'element' => Element::Metal,
                'heal' => null,
                'name' => "Coup d'épée",
                'description' => "Un coup d'épée neutre",
                'hit' => 90,
                'level' => 1,
            ],
            'sharp_blade' => [
                'slug' => 'sharp-blade',
                'register' => CombatRegister::Melee,
                'damage' => 1,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Frappe puissante',
                'description' => 'Une frappe puissante avec une lame tranchante',
                'hit' => 95,
                'energyCost' => 0,
                'critical' => 10,
                'level' => 1,
                // ARC-07c — l'Entaille passe sur un geste de **melee**. Elle
                // etait portee par `magnetic-pull`, de registre `Spell` : un
                // arbre de melee dont le geste marque est un sort ne qualifie
                // pas ses propres passifs (invariant 7, ARC-02b).
                'statusEffectSlug' => 'gash',
            ],
            'metal_storm' => [
                'slug' => 'metal-storm',
                'damage' => 5,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Tempête métallique',
                'description' => 'Une pluie de fragments métalliques acérés',
                'hit' => 80,
                'level' => 3,
            ],
            // ARC-07c — **les trois premieres protections de melee du jeu.**
            // Le § 9.3 veut que tous les accords du Soldat soient des
            // techniques ; il n'en existait aucune qui protege, si bien que
            // l'archetype d'encaisse n'avait rien a jouer sur son tour
            // defensif. Elles portent un bouclier, donc elles **se deposent**
            // (ARC-11b, extension a toute protection) : *un tour paye, trois
            // tours couverts*. Sans cela, une garde punit l'encaisse de se
            // defendre — il perd en degats exactement ce qu'il gagne en survie.
            'garde_haute' => [
                'slug' => 'garde-haute',
                'damage' => null,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Garde haute',
                'description' => 'Un tour payé, trois tours couverts',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 0,
                'statusEffectSlug' => 'shield',
                'critical' => 0,
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::SelfOnly,
            ],
            // **Le depot de l'encaisseur** : une rencontre a PV partages ne se
            // « prend » pas, elle s'amortit. Ce qui agit sur un **etat** se
            // multiplie par le nombre d'allies (§ 7 bis).
            'mur_de_boucliers' => [
                'slug' => 'mur-de-boucliers',
                'damage' => null,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Mur de boucliers',
                'description' => 'Une absorption posée sur chaque allié',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 1,
                'statusEffectSlug' => 'shield',
                'critical' => 0,
                'level' => 2,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::Group,
            ],
            'rempart' => [
                'slug' => 'rempart',
                'damage' => null,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Rempart',
                'description' => 'Ce qu\'on vient chercher en donjon : encaisser pour quatre, et le déposer',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 3,
                'statusEffectSlug' => 'shield-strong',
                'critical' => 0,
                'level' => 4,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::Group,
            ],
            'iron_fist' => [
                'slug' => 'iron-fist',
                'register' => CombatRegister::Melee,
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Charge',
                'description' => 'Une charge brutale avec un poing de fer',
                'hit' => 90,
                'energyCost' => 0,
                'critical' => 12,
                'cooldown' => 1,
                'level' => 2,
            ],
            'blade_dance' => [
                'slug' => 'blade-dance',
                'register' => CombatRegister::Melee,
                'damage' => 6,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Tourbillon d\'epee',
                'description' => 'Un tourbillon devastateur de lames qui frappe tous les ennemis',
                'hit' => 85,
                'energyCost' => 0,
                'cooldown' => 3,
                'critical' => 15,
                'aoeTargets' => 0,
                'level' => 4,
            ],
            'steel_shield' => [
                'slug' => 'steel-shield',
                'damage' => 0,
                'element' => Element::Metal,
                'heal' => 4,
                'name' => 'Bouclier d\'acier',
                'description' => 'Un bouclier métallique qui offre une protection solide',
                'hit' => 100,
                'level' => 2,
            ],
            'razor_edge' => [
                'slug' => 'razor-edge',
                'register' => CombatRegister::Melee,
                'damage' => 7,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Lame rasoir',
                'description' => 'Une lame si fine qu\'elle coupe presque tout',
                'hit' => 80,
                'energyCost' => 0,
                'cooldown' => 3,
                'level' => 4,
            ],
            'metal_skin' => [
                'slug' => 'metal-skin',
                'damage' => 1,
                'element' => Element::Metal,
                'heal' => 5,
                'name' => 'Peau métallique',
                'description' => 'Transforme la peau en métal pour une protection accrue',
                'hit' => 100,
                'level' => 3,
            ],
            'shrapnel_burst' => [
                'slug' => 'shrapnel-burst',
                'damage' => 4,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Explosion d\'éclats',
                'description' => 'Une explosion qui projette des éclats métalliques dans toutes les directions',
                'hit' => 85,
                'level' => 2,
            ],
            'iron_maiden' => [
                'slug' => 'iron-maiden',
                'register' => CombatRegister::Melee,
                'damage' => 8,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Vierge de fer',
                'description' => 'Emprisonne l\'ennemi dans une cage de pointes métalliques',
                'hit' => 75,
                'energyCost' => 0,
                'cooldown' => 4,
                'level' => 5,
            ],
            'chain_lightning' => [
                'slug' => 'chain-lightning',
                'damage' => 5,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Chaîne d\'éclairs',
                'description' => 'Des chaînes métalliques chargées d\'électricité frappent l\'ennemi',
                'hit' => 85,
                'level' => 3,
            ],
            'metallic_regeneration' => [
                'slug' => 'metallic-regeneration',
                'damage' => 0,
                'element' => Element::Metal,
                'heal' => 6,
                'name' => 'Régénération métallique',
                'description' => 'Utilise des particules métalliques pour réparer les blessures',
                'hit' => 100,
                'level' => 3,
            ],
            'blade_barrier' => [
                'slug' => 'blade-barrier',
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => 3,
                'name' => 'Barrière de lames',
                'description' => 'Crée une barrière de lames tournoyantes qui protège et attaque',
                'hit' => 90,
                'level' => 2,
            ],
            'silver_bolt' => [
                'slug' => 'silver-bolt',
                'register' => CombatRegister::Melee,
                'damage' => 6,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Carreau d\'argent',
                'description' => 'Un projectile d\'argent pur qui transperce les défenses',
                'hit' => 85,
                'energyCost' => 0,
                'cooldown' => 2,
                'level' => 3,
            ],
            'crushing_weight' => [
                'slug' => 'crushing-weight',
                'register' => CombatRegister::Melee,
                'damage' => 7,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Poids écrasant',
                'description' => 'Une masse métallique qui s\'abat sur l\'ennemi',
                'hit' => 80,
                'energyCost' => 0,
                'cooldown' => 3,
                'level' => 4,
            ],
            'magnetic_pull' => [
                'slug' => 'magnetic-pull',
                'damage' => 2,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Attraction magnétique',
                'description' => 'Attire violemment les objets métalliques vers l\'ennemi',
                'hit' => 95,
                'level' => 1,
                'statusEffectSlug' => 'gash',
            ],

            // Sorts de nature
            'liana_whip' => [
                'slug' => 'liana-whip',
                'damage' => 1,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Liane',
                'description' => 'Des lianes fouettent l\'ennemi',
                'hit' => 90,
                'energyCost' => 4,
                'level' => 1,
                'statusEffectSlug' => 'hunted',
            ],
            'thorn_burst' => [
                'slug' => 'thorn-burst',
                'damage' => 3,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Explosion d\'épines',
                'description' => 'Des épines jaillissent du sol et blessent les ennemis',
                'hit' => 85,
                'level' => 2,
            ],
            'natural_healing' => [
                'slug' => 'natural-healing',
                'damage' => null,
                'element' => Element::Beast,
                'heal' => 4,
                'name' => 'Guérison naturelle',
                'description' => 'Les forces de la nature soignent les blessures',
                'hit' => 100,
                'level' => 1,
            ],
            'poison_cloud' => [
                'slug' => 'poison-cloud',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Empoisonnement',
                'description' => 'Un nuage de spores toxiques qui empoisonne l\'ennemi',
                'hit' => 85,
                'energyCost' => 10,
                'statusEffectSlug' => 'poison',
                'level' => 3,
            ],
            'entangling_roots' => [
                'slug' => 'entangling-roots',
                'damage' => 2,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Racines enchevetrees',
                'description' => 'Des racines surgissent du sol et immobilisent l\'ennemi',
                'hit' => 95,
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
            ],
            'nature_wrath' => [
                'slug' => 'nature-wrath',
                'damage' => 6,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Appel de la foret',
                'description' => 'Dechaine la fureur de la nature sur l\'ennemi',
                'hit' => 80,
                'energyCost' => 20,
                'cooldown' => 3,
                'statusEffectSlug' => 'poison-strong',
                'aoeTargets' => 0,
                'level' => 4,
            ],
            'leaf_blade' => [
                'slug' => 'leaf-blade',
                'damage' => 5,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Lame feuille',
                'description' => 'Une lame tranchante formée de feuilles acérées',
                'hit' => 90,
                'level' => 3,
            ],
            'forest_embrace' => [
                'slug' => 'forest-embrace',
                'damage' => null,
                'element' => Element::Beast,
                'heal' => 7,
                'name' => 'Étreinte de la forêt',
                'description' => 'L\'énergie de la forêt enveloppe et soigne les blessures',
                'hit' => 100,
                'level' => 3,
            ],
            'venomous_bite' => [
                'slug' => 'venomous-bite',
                'damage' => 3,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Morsure venimeuse',
                'description' => 'Une morsure qui injecte un venin puissant',
                'hit' => 90,
                'level' => 2,
                'statusEffectSlug' => 'hunted',
            ],
            'wild_growth' => [
                'slug' => 'wild-growth',
                'damage' => 1,
                'element' => Element::Beast,
                'heal' => 5,
                'name' => 'Croissance sauvage',
                'description' => 'Des plantes médicinales poussent rapidement et soignent les blessures',
                'hit' => 100,
                'level' => 2,
            ],
            'nature_fury' => [
                'slug' => 'nature-fury',
                'damage' => 8,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Fureur naturelle',
                'description' => 'Libère toute la puissance destructrice de la nature',
                'hit' => 75,
                'level' => 5,
            ],
            'thorn_shield' => [
                'slug' => 'thorn-shield',
                'damage' => 2,
                'element' => Element::Beast,
                'heal' => 3,
                'name' => 'Bouclier d\'épines',
                'description' => 'Un bouclier d\'épines qui protège et blesse les attaquants',
                'hit' => 100,
                'level' => 2,
                'statusEffectSlug' => 'hunted',
            ],
            'toxic_spores' => [
                'slug' => 'toxic-spores',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Spores toxiques',
                'description' => 'Libère des spores hautement toxiques dans l\'air',
                'hit' => 85,
                'level' => 2,
            ],
            'nature_blessing' => [
                'slug' => 'nature-blessing',
                'damage' => null,
                'element' => Element::Beast,
                'heal' => 10,
                'name' => 'Bénédiction de la nature',
                'description' => 'La bénédiction de la nature qui restaure la vitalité',
                'hit' => 100,
                'level' => 4,
            ],
            'vine_snare' => [
                'slug' => 'vine-snare',
                'damage' => 3,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Piège de vignes',
                'description' => 'Des vignes surgissent et s\'enroulent autour de l\'ennemi',
                'hit' => 90,
                'level' => 2,
            ],
            'primal_surge' => [
                'slug' => 'primal-surge',
                'damage' => 5,
                'element' => Element::Beast,
                'heal' => 5,
                'name' => 'Afflux primordial',
                'description' => 'Un afflux d\'énergie primordiale qui soigne et blesse',
                'hit' => 85,
                'level' => 4,
            ],

            // Sorts de vent
            'wind_lame' => [
                'slug' => 'wind-lame',
                'damage' => 1,
                'element' => Element::Air,
                'heal' => null,
                'name' => "Lame d'air",
                'description' => "une lame d'air tranchante",
                'hit' => 90,
                'level' => 1,
            ],
            'tornado' => [
                'slug' => 'tornado',
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tornade',
                'description' => 'Une violente tornade qui emporte tout sur son passage',
                'hit' => 80,
                'level' => 2,
            ],
            'wind_shield' => [
                'slug' => 'wind-shield',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 2,
                'name' => 'Bouclier de vent',
                'description' => 'Un souffle qui detourne ce qui vient',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 0,
                'statusEffectSlug' => 'shield',
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::SelfOnly,
            ],
            'gust' => [
                'slug' => 'gust',
                'damage' => 2,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Bourrasque',
                'description' => 'Une puissante bourrasque qui repousse l\'ennemi',
                'hit' => 95,
                'level' => 1,
                'statusEffectSlug' => 'off-balance',
            ],
            'air_dash' => [
                'slug' => 'air-dash',
                'register' => CombatRegister::Ranged,
                'damage' => 1,
                'element' => Element::Air,
                'heal' => 1,
                'name' => 'Ruée d\'air',
                'description' => 'Permet de se déplacer rapidement sur un coussin d\'air',
                'hit' => 100,
                'energyCost' => 0,
                'ammoCost' => 1,
                'level' => 1,
            ],
            'air_slash' => [
                'slug' => 'air-slash',
                'register' => CombatRegister::Ranged,
                'damage' => 5,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tranchant aérien',
                'description' => 'Une lame d\'air comprimé qui tranche l\'ennemi',
                'hit' => 85,
                'energyCost' => 0,
                'ammoCost' => 3,
                'level' => 3,
            ],
            'cyclone' => [
                'slug' => 'cyclone',
                'damage' => 6,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Cyclone',
                'description' => 'Un puissant cyclone qui déchire tout sur son passage',
                'hit' => 80,
                'level' => 3,
            ],
            'wind_blast' => [
                'slug' => 'wind-blast',
                'damage' => 3,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Souffle du vent',
                'description' => 'Une rafale concentrée qui frappe l\'ennemi',
                'hit' => 90,
                'level' => 2,
            ],
            'healing_breeze' => [
                'slug' => 'healing-breeze',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 5,
                'name' => 'Brise apaisante',
                'description' => 'Le vent passe sur les siens et referme ce qui saigne',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 1,
                'level' => 2,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Heal,
                'scope' => SpellScope::Ally,
            ],
            'vacuum_blade' => [
                'slug' => 'vacuum-blade',
                'damage' => 7,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Lame de vide',
                'description' => 'Une lame créée par le vide qui tranche tout',
                'hit' => 75,
                'level' => 4,
            ],
            'wind_wall' => [
                'slug' => 'wind-wall',
                'damage' => 1,
                'element' => Element::Air,
                'heal' => 4,
                'name' => 'Mur de vent',
                'description' => 'Un mur de vent qui protège et repousse les attaques',
                'hit' => 100,
                'level' => 2,
            ],
            'hurricane' => [
                'slug' => 'hurricane',
                'damage' => 8,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Ouragan',
                'description' => 'Un ouragan dévastateur qui ravage tout',
                'hit' => 70,
                'level' => 5,
            ],
            'air_current' => [
                'slug' => 'air-current',
                // Il devient l'accord de branche du Contre, donc un geste de
                // **palier 3** ; ses 2 degats le laissaient sous la mediane de
                // son nouveau palier, et le cliquet d'ARC-05a l'a vu. *Changer
                // le palier d'un geste sans changer sa valeur, c'est l'affaiblir
                // sans le dire.*
                'damage' => 5,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Courant d\'air',
                'description' => 'Une lame d\'air qui passe entre les gardes',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 2,
                'level' => 3,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            // ARC-07d — **la Volee** : le geste a plusieurs cibles de l'Archer.
            // Ses deux munitions ne sont pas un choix d'auteur, elles sont la
            // grille du registre a son palier (GAME_MATERIA § 2.3 bis) : *un
            // geste ne facture que la ressource de son registre*, et cette
            // ressource se lit sur le palier, jamais sur la portee.
            'pressure_point' => [
                'slug' => 'pressure-point',
                'register' => CombatRegister::Ranged,
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Volée',
                'description' => 'Deux traits partent ensemble et se separent en vol',
                'hit' => 90,
                'energyCost' => 0,
                'aoeTargets' => 2,
                'ammoCost' => 2,
                'level' => 2,
            ],
            'wind_scythe' => [
                'slug' => 'wind-scythe',
                'damage' => 6,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Faux de vent',
                'description' => 'Une faux d\'air tranchante qui fauche les ennemis',
                'hit' => 85,
                'level' => 3,
            ],
            'tempest' => [
                'slug' => 'tempest',
                'damage' => 5,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tempête',
                'description' => 'Une violente tempête qui frappe de toutes parts',
                'hit' => 85,
                'level' => 3,
            ],
            'wind_blessing' => [
                'slug' => 'wind-blessing',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 7,
                'name' => 'Bénédiction du vent',
                'description' => 'La bénédiction des vents qui soigne et revigore',
                'hit' => 100,
                'level' => 3,
            ],

            // Sorts d'eau
            'water_jet' => [
                'slug' => 'water-jet',
                'damage' => 2,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Jet d\'eau',
                'description' => 'Un jet d\'eau sous pression qui frappe l\'ennemi',
                'hit' => 90,
                'energyCost' => 4,
                'level' => 1,
                'statusEffectSlug' => 'soaked',
            ],
            'tidal_wave' => [
                'slug' => 'tidal-wave',
                'damage' => 5,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Raz-de-marée',
                'description' => 'Une vague massive qui submerge les ennemis',
                'hit' => 80,
                'energyCost' => 12,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'frost_bolt' => [
                'slug' => 'frost-bolt',
                'damage' => 3,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Trait de givre',
                'description' => 'Un projectile de glace qui ralentit la cible',
                'hit' => 90,
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
            ],
            'aqua_shield' => [
                'slug' => 'aqua-shield',
                'damage' => 0,
                'element' => Element::Water,
                'heal' => 4,
                'name' => 'Bouclier aquatique',
                'description' => 'Un bouclier d\'eau qui absorbe les coups',
                'hit' => 100,
                'energyCost' => 6,
                'statusEffectSlug' => 'shield',
                'level' => 1,
            ],
            'tsunami' => [
                'slug' => 'tsunami',
                'damage' => 8,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Tsunami',
                'description' => 'Un tsunami dévastateur qui engloutit tout',
                'hit' => 70,
                'energyCost' => 25,
                'cooldown' => 4,
                'aoeTargets' => 0,
                'critical' => 12,
                'level' => 5,
            ],
            'maelstrom' => [
                'slug' => 'maelstrom',
                'damage' => 6,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Maelström',
                'description' => 'Un tourbillon aquatique qui aspire et blesse les ennemis',
                'hit' => 80,
                'energyCost' => 15,
                'cooldown' => 3,
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // ARC-07b — **les deux premiers gestes de portee `le groupe` du
            // jeu.** La loi du depot (ARC-11b) etait ecrite, opposable, et sans
            // objet : aucun geste ne portait `SpellScope::Group`, qui **ne se
            // derive jamais** (un soin de groupe et un soin d'allie ont
            // exactement les memes valeurs — ce qui les separe est une decision
            // d'auteur). Les voici.
            //
            // Ils portent un statut de regeneration, et c'est ce qui les rend
            // deposables : un geste de groupe **instantane** est interdit
            // (`DepositedGestureContractTest`), parce qu'un soin reactif ne
            // sert a rien dans un donjon semi-synchrone ou le tour d'un absent
            // se resout tout seul. *Le soigneur ne soigne pas, il provisionne.*
            'maree' => [
                'slug' => 'maree',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 3,
                'name' => 'Marée',
                'description' => 'Une provision posée sur tout le groupe, qui court même en votre absence',
                'hit' => 100,
                'energyCost' => 14,
                'statusEffectSlug' => 'regeneration',
                'critical' => 0,
                'level' => 2,
                'intent' => SpellIntent::Heal,
                'scope' => SpellScope::Group,
            ],
            // La branche Maree pousse la meme idee jusqu'a couvrir une
            // rencontre entiere. La duree **etale** la valeur, elle ne
            // l'augmente pas (ARC-11b) : allonger un depot achete de la
            // robustesse a l'absence, jamais de la puissance.
            'grande_maree' => [
                'slug' => 'grande-maree',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 5,
                'name' => 'Grande Marée',
                'description' => 'Le dépôt qui couvre une rencontre entière',
                'hit' => 100,
                'energyCost' => 26,
                'statusEffectSlug' => 'regeneration-strong',
                'critical' => 0,
                'level' => 4,
                'intent' => SpellIntent::Heal,
                'scope' => SpellScope::Group,
            ],
            'water_heal' => [
                'slug' => 'water-heal',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 5,
                'name' => 'Soin aquatique',
                'description' => 'L\'eau purificatrice soigne les blessures',
                'hit' => 100,
                'energyCost' => 5,
                'level' => 1,
            ],
            'mist_veil' => [
                'slug' => 'mist-veil',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 6,
                'name' => 'Voile de brume',
                'description' => 'Une brume protectrice qui soigne et cache',
                'hit' => 100,
                'energyCost' => 8,
                'statusEffectSlug' => 'shield',
                'level' => 2,
            ],
            'ice_lance' => [
                'slug' => 'ice-lance',
                'damage' => 4,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Lance de glace',
                'description' => 'Une lance de glace qui transperce l\'ennemi',
                'hit' => 85,
                'energyCost' => 8,
                'critical' => 10,
                'level' => 2,
            ],
            'water_prison' => [
                'slug' => 'water-prison',
                'damage' => 3,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Prison d\'eau',
                'description' => 'Emprisonne l\'ennemi dans une sphère d\'eau',
                'hit' => 90,
                'statusEffectSlug' => 'paralysis',
                'level' => 3,
            ],
            'ocean_blessing' => [
                'slug' => 'ocean-blessing',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 10,
                'name' => 'Bénédiction de l\'océan',
                'description' => 'Les eaux sacrées restaurent profondément la vitalité',
                'hit' => 100,
                'energyCost' => 15,
                'statusEffectSlug' => 'regeneration',
                'level' => 4,
            ],
            'ice_storm' => [
                'slug' => 'ice-storm',
                'damage' => 5,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Tempête de glace',
                'description' => 'Une tempête de grêle et de glace qui martèle la zone',
                'hit' => 80,
                'energyCost' => 14,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'bubble_shield' => [
                'slug' => 'bubble-shield',
                'damage' => 0,
                'element' => Element::Water,
                'heal' => 8,
                'name' => 'Bulle protectrice',
                'description' => 'Une bulle d\'eau qui protège des attaques',
                'hit' => 100,
                'energyCost' => 12,
                'statusEffectSlug' => 'shield-strong',
                'level' => 3,
            ],
            'torrent' => [
                'slug' => 'torrent',
                'damage' => 4,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Torrent',
                'description' => 'Un torrent d\'eau qui emporte tout sur son passage',
                'hit' => 85,
                'level' => 2,
            ],
            'frozen_touch' => [
                'slug' => 'frozen-touch',
                'damage' => 2,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Toucher glacé',
                'description' => 'Un toucher qui gèle la cible au contact',
                'hit' => 95,
                'level' => 1,
            ],
            'rising_tide' => [
                'slug' => 'rising-tide',
                'damage' => 1,
                'element' => Element::Water,
                'heal' => 3,
                'name' => 'Marée montante',
                'description' => 'La marée monte et soigne les alliés tout en repoussant les ennemis',
                'hit' => 95,
                'level' => 2,
                'statusEffectSlug' => 'soaked',
            ],

            // Sorts spécifiques — Berserker (feu/physique)
            'rage_flame' => [
                'slug' => 'rage-flame',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Flamme de rage',
                'description' => 'La rage intérieure se transforme en flammes destructrices',
                'hit' => 95,
                'energyCost' => 6,
                'critical' => 15,
                'level' => 2,
                'statusEffectSlug' => 'burn',
            ],
            'berserk_charge' => [
                'slug' => 'berserk-charge',
                'damage' => 4,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Charge enflammée',
                'description' => 'Une charge furieuse enveloppée de flammes',
                'hit' => 90,
                'energyCost' => 10,
                'critical' => 12,
                'level' => 3,
            ],
            'fury_strike' => [
                'slug' => 'fury-strike',
                'damage' => 6,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Frappe de furie',
                'description' => 'Un coup dévastateur porté par une rage incontrôlable',
                'hit' => 80,
                'energyCost' => 15,
                'critical' => 18,
                'level' => 4,
            ],
            'blood_fury' => [
                'slug' => 'blood-fury',
                'damage' => 9,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Furie sanguinaire',
                'description' => 'La rage atteint son paroxysme, déchaînant une puissance dévastatrice',
                'hit' => 75,
                'energyCost' => 25,
                'cooldown' => 4,
                'critical' => 20,
                'level' => 5,
            ],

            // Sorts spécifiques — Artificier (feu/pièges)
            'fire_trap' => [
                'slug' => 'fire-trap',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Piège incendiaire',
                'description' => 'Pose un piège qui explose en flammes au contact',
                'hit' => 95,
                // ARC-08c : le tir ne paie pas de PM, il paie sa munition
                // (ARC-04b). Le carquois est **durable** — il se vide dans la
                // rencontre et se ramasse apres —, donc ce cout n'est jamais un
                // cout en gils.
                'energyCost' => 0,
                'ammoCost' => 2,
                'statusEffectSlug' => 'burn',
                'level' => 2,
                'register' => CombatRegister::Ranged,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            'explosive_mine' => [
                'slug' => 'explosive-mine',
                'damage' => 5,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Mine explosive',
                'description' => 'Une mine qui explose violemment au contact',
                'hit' => 90,
                'energyCost' => 10,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'flash_bomb' => [
                'slug' => 'flash-bomb',
                'damage' => 2,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Bombe flash',
                'description' => 'Une bombe aveuglante qui désoriente les ennemis',
                'hit' => 100,
                'energyCost' => 0,
                'ammoCost' => 2,
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
                'aoeTargets' => 3,
                'register' => CombatRegister::Ranged,
                // **L'intention se declare, elle ne se derive pas.** La
                // derivation lit le degat d'abord (ARC-11a), donc ce geste se
                // rangeait en `degat` alors qu'on ne le lance pas pour ses deux
                // points : on le lance pour la paralysie. C'est le meme constat
                // qu'ARC-08a a fait sur les entraves du Necromancien.
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Targets,
            ],
            'artillery_barrage' => [
                'slug' => 'artillery-barrage',
                'damage' => 8,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Barrage d\'artillerie',
                'description' => 'Un bombardement massif qui dévaste la zone',
                'hit' => 70,
                'energyCost' => 0,
                'ammoCost' => 5,
                // La reprise reste : GAME_MATERIA § 2.3 bis regle 3 autorise un
                // `cooldown` hors melee **a partir du palier 3**, comme
                // garde-fou anti-spam. Celui-ci est de palier 5.
                'cooldown' => 4,
                'aoeTargets' => 0,
                'critical' => 10,
                'level' => 5,
                'register' => CombatRegister::Ranged,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Targets,
            ],

            // --- Les deux entraves que l'Artificier devait ecrire (ARC-08c) ---
            //
            // La palette du **controle** reclame au moins deux accords
            // d'`entrave` (§ 5.1), et l'arbre n'en portait qu'un seul candidat.
            // Les gestes de feu a plusieurs cibles deja livres — Mur de feu,
            // Nova de feu, Pluie de flammes — sont **partages avec le
            // Pyromancien**, donc de registre `spell` : les lui prendre casserait
            // un arbre livre, et les partager obligerait un meme geste a etre une
            // technique ici et un sort la-bas. C'est la regle qu'ARC-08b a
            // enoncee, appliquee une seconde fois.
            'pitch_slick' => [
                'slug' => 'pitch-slick',
                'damage' => 2,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Nappe de poix',
                'description' => 'Le sol prend, et qui le traverse prend avec lui',
                'hit' => 100,
                'energyCost' => 0,
                'ammoCost' => 3,
                'statusEffectSlug' => 'burn',
                'level' => 3,
                'aoeTargets' => 3,
                'register' => CombatRegister::Ranged,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Targets,
            ],
            'covering_fire' => [
                'slug' => 'covering-fire',
                'damage' => 2,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Tir couvrant',
                'description' => 'Personne n\'avance tant que la ligne tient',
                'hit' => 100,
                'energyCost' => 0,
                'ammoCost' => 4,
                'statusEffectSlug' => 'paralysis',
                'level' => 4,
                'aoeTargets' => 3,
                'register' => CombatRegister::Ranged,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Targets,
            ],

            // Sorts spécifiques — Archer (air/physique distance)
            'precise_shot' => [
                'slug' => 'precise-shot',
                'register' => CombatRegister::Ranged,
                'damage' => 2,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tir entravant',
                'description' => 'Un trait qui ne cherche pas a tuer : il fait perdre la ligne',
                'hit' => 100,
                'energyCost' => 0,
                'critical' => 12,
                'ammoCost' => 1,
                'level' => 1,
                'statusEffectSlug' => 'off-balance',
                'intent' => SpellIntent::Hinder,
            ],
            'arrow_rain' => [
                'slug' => 'arrow-rain',
                'register' => CombatRegister::Ranged,
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Grêle',
                'description' => 'Trois traits en cloche, et la moitie se ramasse apres',
                'hit' => 80,
                'energyCost' => 0,
                'aoeTargets' => 3,
                'ammoCost' => 3,
                'level' => 3,
            ],
            'critical_shot' => [
                'slug' => 'critical-shot',
                'register' => CombatRegister::Ranged,
                'damage' => 6,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tir critique',
                'description' => 'Un tir visant les points faibles de l\'ennemi',
                'hit' => 85,
                'energyCost' => 0,
                'critical' => 25,
                'ammoCost' => 3,
                'level' => 3,
            ],
            'piercing_arrow' => [
                'slug' => 'piercing-arrow',
                'register' => CombatRegister::Ranged,
                'damage' => 8,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Flèche perforante',
                'description' => 'Une flèche qui transperce les armures les plus solides',
                'hit' => 80,
                'energyCost' => 0,
                'cooldown' => 3,
                'critical' => 15,
                'ammoCost' => 5,
                'level' => 5,
            ],

            // Sorts spécifiques — Chevalier (métal/tank)
            'provocation' => [
                'slug' => 'provocation',
                'damage' => 1,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Provocation',
                'description' => 'Provoque l\'ennemi, attirant son attention',
                'hit' => 100,
                'energyCost' => 3,
                'level' => 1,
                'statusEffectSlug' => 'gash',
            ],
            'riposte' => [
                'slug' => 'riposte',
                'damage' => 4,
                'element' => Element::Metal,
                'heal' => 2,
                'name' => 'Riposte',
                'description' => 'Contre-attaque après une parade réussie',
                'hit' => 95,
                'energyCost' => 8,
                'level' => 2,
            ],
            'steel_fortress' => [
                'slug' => 'steel-fortress',
                'damage' => 0,
                'element' => Element::Metal,
                'heal' => 10,
                'name' => 'Forteresse d\'acier',
                'description' => 'Se transforme en forteresse imprenable',
                'hit' => 100,
                'energyCost' => 20,
                'cooldown' => 4,
                'statusEffectSlug' => 'shield-strong',
                'level' => 5,
            ],

            // Sorts spécifiques — Ingénieur (métal/support)
            'turret' => [
                'slug' => 'turret',
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Tourelle',
                'description' => 'Déploie une tourelle métallique qui tire sur l\'ennemi',
                'hit' => 85,
                'energyCost' => 8,
                'level' => 2,
            ],
            'repair_bot' => [
                'slug' => 'repair-bot',
                'damage' => null,
                'element' => Element::Metal,
                'heal' => 6,
                'name' => 'Automate réparateur',
                'description' => 'Un petit automate qui répare les blessures',
                'hit' => 100,
                'energyCost' => 10,
                'level' => 3,
            ],
            'siege_engine' => [
                'slug' => 'siege-engine',
                'damage' => 7,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Engin de siège',
                'description' => 'Déploie un puissant engin de siège',
                'hit' => 80,
                'energyCost' => 20,
                'cooldown' => 4,
                'aoeTargets' => 0,
                'level' => 5,
            ],

            // Sorts spécifiques — Chasseur (bête/distance)
            'falcon_strike' => [
                'slug' => 'falcon-strike',
                'damage' => 2,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Appel du faucon',
                'description' => 'Un faucon fond sur l\'ennemi depuis les cieux',
                'hit' => 95,
                'energyCost' => 5,
                'level' => 1,
            ],
            'bear_trap' => [
                'slug' => 'bear-trap',
                'damage' => 3,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Piège à ours',
                'description' => 'Un piège qui immobilise et blesse la cible',
                'hit' => 90,
                'energyCost' => 7,
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
            ],
            'poison_arrow' => [
                'slug' => 'poison-arrow',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Tir empoisonné',
                'description' => 'Une flèche enduite de poison qui affaiblit la cible',
                'hit' => 85,
                'energyCost' => 9,
                'statusEffectSlug' => 'poison',
                'level' => 3,
            ],
            'pack_hunt' => [
                'slug' => 'pack-hunt',
                'damage' => 7,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Chasse en meute',
                'description' => 'Appelle la meute pour une attaque coordonnée',
                'hit' => 80,
                'energyCost' => 20,
                'cooldown' => 4,
                'aoeTargets' => 0,
                'level' => 5,
            ],

            // Sorts spécifiques — Dompteur (bête/tank invocateur)
            'beast_bond' => [
                'slug' => 'beast-bond',
                'damage' => 1,
                'element' => Element::Beast,
                'heal' => 3,
                'name' => 'Lien bestial',
                'description' => 'Crée un lien avec une bête qui protège et soigne',
                'hit' => 100,
                'energyCost' => 5,
                'level' => 1,
            ],
            'savage_charge' => [
                'slug' => 'savage-charge',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Charge sauvage',
                'description' => 'La bête liée charge furieusement l\'ennemi',
                'hit' => 90,
                'energyCost' => 10,
                'critical' => 12,
                'level' => 3,
            ],
            'alpha_roar' => [
                'slug' => 'alpha-roar',
                'damage' => 2,
                'element' => Element::Beast,
                'heal' => 5,
                'name' => 'Rugissement alpha',
                'description' => 'Un rugissement qui terrifie les ennemis et renforce les alliés',
                'hit' => 95,
                'energyCost' => 12,
                'statusEffectSlug' => 'paralysis',
                'level' => 4,
            ],

            // Sorts spécifiques — Paladin (lumière/tank healer)
            'sacred_strike' => [
                'slug' => 'sacred-strike',
                'damage' => 3,
                'element' => Element::Light,
                'heal' => 2,
                'name' => 'Frappe sacrée',
                'description' => 'Un coup béni qui blesse et purifie',
                'hit' => 90,
                'energyCost' => 6,
                'level' => 2,
                'statusEffectSlug' => 'revealed',
            ],
            'light_aura' => [
                'slug' => 'light-aura',
                'damage' => 0,
                'element' => Element::Light,
                'heal' => 4,
                'name' => 'Aura de lumière',
                'description' => 'Une aura protectrice qui soigne continuellement',
                'hit' => 100,
                'energyCost' => 8,
                'statusEffectSlug' => 'regeneration',
                'level' => 2,
            ],
            'divine_judgment' => [
                'slug' => 'divine-judgment',
                'damage' => 7,
                'element' => Element::Light,
                'heal' => 3,
                'name' => 'Jugement divin',
                'description' => 'La lumière divine frappe les impurs et guérit les justes',
                'hit' => 85,
                'energyCost' => 20,
                'cooldown' => 4,
                'critical' => 12,
                'level' => 5,
            ],

            // Sorts spécifiques — Prêtre (lumière/heal pur)
            'prayer' => [
                'slug' => 'prayer',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 6,
                'name' => 'Prière',
                'description' => 'Une prière fervente qui restaure la santé',
                'hit' => 100,
                'energyCost' => 7,
                'level' => 2,
            ],
            'miracle' => [
                'slug' => 'miracle',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 15,
                'name' => 'Miracle',
                'description' => 'Un miracle divin qui restaure massivement la santé',
                'hit' => 100,
                'energyCost' => 25,
                'cooldown' => 5,
                'statusEffectSlug' => 'regeneration-strong',
                'level' => 5,
            ],

            // Sorts spécifiques — Inquisiteur (lumière/DPS)
            'smite' => [
                'slug' => 'smite',
                'damage' => 3,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Châtiment sacré',
                'description' => 'Frappe l\'ennemi avec la colère divine',
                'hit' => 90,
                'energyCost' => 6,
                'level' => 2,
            ],
            'holy_fire' => [
                'slug' => 'holy-fire',
                'damage' => 5,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Feu sacré',
                'description' => 'Des flammes sacrées qui brûlent les impurs',
                'hit' => 85,
                'energyCost' => 10,
                'statusEffectSlug' => 'burn',
                'level' => 3,
            ],
            'divine_sentence' => [
                'slug' => 'divine-sentence',
                'damage' => 8,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Sentence divine',
                'description' => 'Le jugement final frappe l\'hérétique sans pitié',
                'hit' => 80,
                'energyCost' => 22,
                'cooldown' => 4,
                'critical' => 15,
                'level' => 5,
            ],

            // Sorts spécifiques — Assassin (ombre/CaC furtif)
            'ambush' => [
                'slug' => 'ambush',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Embuscade',
                'description' => 'Surgit de l\'ombre pour frapper par surprise',
                'hit' => 95,
                // ARC-08b : la melee ne paie pas de PM, elle paie le tour
                // suivant (ARC-04a). Un accord d'entree est gratuit **des deux
                // facons** — c'est la regle du jour 1 (GAME_MATERIA § 3), et
                // c'est elle qui rend le capstone atteignable des la premiere
                // rencontre : ce geste pose Aveugle sans rien couter.
                'energyCost' => 0,
                'cooldown' => 0,
                // Palier **1** et non 2 : la grille de reprise rend le palier 1
                // gratuit, et c'est la seule facon de tenir ensemble les deux
                // regles qui portent sur ce geste — *un accord d'entree est
                // gratuit* (GAME_MATERIA § 3) et *au-dela du palier 1, une
                // technique coute toujours un tour*. Un geste distribue a 0
                // point qui se declarait de palier 2 les mettait en conflit.
                'level' => 1,
                'statusEffectSlug' => 'blinded',
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            'deadly_strike' => [
                'slug' => 'deadly-strike',
                'damage' => 6,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Coup mortel',
                'description' => 'Un coup visant les organes vitaux',
                'hit' => 80,
                'energyCost' => 0,
                'cooldown' => 3,
                'critical' => 25,
                'level' => 4,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            // --- Les deux gestes que l'Assassin devait ecrire (ARC-08b) ------
            //
            // Le gabarit lui demande un accord de **protection** au palier 1 et
            // un accord **a plusieurs cibles** au palier 2. Aucun des deux
            // n'existait en registre melee : les gestes sombres livres sont
            // tous des sorts, et GAME_TREE_ANATOMY § 4.7 nommait « Nova de
            // mort » pour la seconde case — un geste que le Necromancien
            // ouvre deja, donc **de registre `spell`**. Le lui prendre aurait
            // casse un arbre livre ; le partager aurait oblige un meme geste a
            // etre une technique ici et un sort la-bas.
            //
            // C'est la lecon d'ARC-07d, transposee : *un accord partage garde
            // le registre de celui qui l'a ouvert le premier*, et l'arbre qui
            // arrive apres ecrit le sien.
            'shadow_veil' => [
                'slug' => 'shadow-veil',
                'damage' => null,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Voile',
                'description' => 'Un pas dans l\'ombre, et le coup ne trouve personne',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 1,
                'statusEffectSlug' => 'shield',
                'critical' => 0,
                'level' => 2,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::SelfOnly,
            ],
            'shadow_reap' => [
                'slug' => 'shadow-reap',
                'damage' => 5,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Fauchée d\'ombre',
                'description' => 'La lame passe bas et large, et personne ne l\'a vue venir',
                'hit' => 85,
                'energyCost' => 0,
                'cooldown' => 2,
                'critical' => 15,
                'level' => 3,
                'aoeTargets' => 3,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Targets,
            ],
            'shadow_dance' => [
                'slug' => 'shadow-dance',
                'damage' => 8,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Danse des ombres',
                'description' => 'Se fond dans les ombres pour porter une série de coups mortels',
                'hit' => 85,
                'energyCost' => 0,
                'cooldown' => 4,
                'critical' => 20,
                'level' => 5,
                'aoeTargets' => 3,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Targets,
            ],

            // Sorts spécifiques — Sorcier (ombre/debuff)
            'hex' => [
                'slug' => 'hex',
                'damage' => 2,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Maléfice',
                'description' => 'Un maléfice qui affaiblit la cible',
                'hit' => 95,
                'energyCost' => 5,
                'statusEffectSlug' => 'poison',
                'level' => 1,
            ],
            'terror' => [
                'slug' => 'terror',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Terreur',
                'description' => 'Plonge l\'ennemi dans une terreur paralysante',
                'hit' => 90,
                'energyCost' => 8,
                'statusEffectSlug' => 'paralysis',
                'level' => 3,
            ],
            'dark_pact' => [
                'slug' => 'dark-pact',
                'damage' => 7,
                'element' => Element::Dark,
                'heal' => 3,
                'name' => 'Pacte sombre',
                'description' => 'Scelle un pacte avec les ténèbres, sacrifiant sa vie pour du pouvoir',
                'hit' => 85,
                'energyCost' => 18,
                'cooldown' => 4,
                'critical' => 12,
                'level' => 5,
            ],

            // Sorts spécifiques — Vagabond (air/support)
            'haste' => [
                'slug' => 'haste',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 3,
                'name' => 'Hâte',
                'description' => 'Accélère les mouvements et restaure un peu d\'énergie',
                'hit' => 100,
                'energyCost' => 5,
                'level' => 1,
            ],
            // --- Les deux gestes du Vagabond (ARC-08e) -----------------------
            //
            // La marque de l'air — Desequilibre — est portee par la Bourrasque,
            // un geste **du Foudromancien** donc de registre `spell`. Quatrieme
            // application de la regle d'ARC-08b : le Vagabond ecrit le sien.
            //
            // Et il lui fallait une **seconde entrave** : la palette du controle
            // en exige deux, l'arbre n'en portait aucune — c'est ce qui le
            // maintenait dans deux listes d'attente a la fois.
            'trip' => [
                'slug' => 'trip',
                'damage' => 2,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Croc-en-jambe',
                'description' => 'Un appui retire au bon moment vaut mieux qu\'un coup',
                'hit' => 95,
                'energyCost' => 0,
                'cooldown' => 0,
                'statusEffectSlug' => 'off-balance',
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Damage,
                'scope' => SpellScope::Target,
            ],
            'headwind' => [
                'slug' => 'headwind',
                // **4 et non 2 : le cliquet d'ARC-05a l'a demande.** Un geste
                // ecrit sous la mediane de son palier creuse l'ecart a l'ancre
                // (§ 6.4) — le releve est passe de x7,6 a x9,5 sur le palier 3
                // avant cette correction. Le cliquet n'est pas un obstacle,
                // c'est ce qui empeche d'ajouter un geste sous-calibre sans le
                // voir : *reduire l'ecart est libre, l'aggraver demande de le
                // dire*. On le pose donc a la mediane de son palier, qui est 5.
                'damage' => 5,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Vent contraire',
                'description' => 'Avancer devient une affaire, pour tout le monde en face',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 2,
                'statusEffectSlug' => 'paralysis',
                'level' => 3,
                'aoeTargets' => 3,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Targets,
            ],
            'mirage' => [
                'slug' => 'mirage',
                'damage' => 2,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Mirage',
                'description' => 'On frappe la ou il etait, et il n\'y est plus',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 1,
                // **L'intention se declare.** La derivation lit le degat
                // d'abord (ARC-11a), donc ce geste se rangeait en `degat` ;
                // on ne le lance pas pour ses deux points, on le lance pour le
                // tour qu'il fait perdre.
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Hinder,
                'scope' => SpellScope::Target,
            ],
            'zephyr' => [
                'slug' => 'zephyr',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 8,
                'name' => 'Zéphyr',
                'description' => 'Un vent doux et chaud qui revigore profondément',
                'hit' => 100,
                'energyCost' => 15,
                'statusEffectSlug' => 'regeneration',
                'level' => 4,
            ],

            // Sorts spécifiques — Gardien (terre/tank support)
            'shared_shield' => [
                'slug' => 'shared-shield',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 5,
                'name' => 'Bouclier partagé',
                'description' => 'Un bouclier de roche qui protège le groupe',
                'hit' => 100,
                'energyCost' => 10,
                'statusEffectSlug' => 'shield',
                'level' => 2,
            ],
            'bastion' => [
                'slug' => 'bastion',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 12,
                'name' => 'Bastion',
                'description' => 'Érige un bastion de pierre indestructible',
                'hit' => 100,
                'energyCost' => 25,
                'cooldown' => 5,
                'statusEffectSlug' => 'shield-strong',
                'level' => 5,
            ],

            // ONB-20a — les mains nues.
            //
            // Le repli qui garantit qu'aucun chemin de combat ne peut echouer
            // faute d'arme. Volontairement le sort le plus faible du jeu :
            // sans element (on ne frappe rien de particulier), sans effet de
            // statut, sans critique, et sans emplacement de materia puisqu'il
            // n'est porte par aucun objet. Les mains nues ne sont pas une arme
            // de secours qu'on choisirait — elles sont ce qui reste.
            //
            // `hit` est ici sans effet : `BareHandsAttack` roule sa propre
            // chance de toucher, celle d'un geste sans entrainement.
            'bare_hands' => [
                'slug' => 'bare-hands',
                'damage' => 1,
                'element' => Element::None,
                'heal' => null,
                'name' => 'Mains nues',
                'description' => 'Faute de mieux, on frappe avec ce qu\'on a.',
                'hit' => 75,
                'critical' => 0,
                'level' => 1,
            ],

            // Sorts de consommables
            'beer_heal' => [
                'slug' => 'beer-heal',
                'damage' => null,
                'element' => Element::None,
                'heal' => 3,
                'name' => 'Gorgée de bière',
                'description' => 'Une bonne chope de bière qui réchauffe et revigore',
                'hit' => 100,
                'level' => 1,
            ],
            'mushroom_heal' => [
                'slug' => 'mushroom-heal',
                'damage' => null,
                'element' => Element::Beast,
                'heal' => 1,
                'name' => 'Champignon comestible',
                'description' => 'Un champignon aux vertus curatives... ou pas',
                'hit' => 100,
                'level' => 1,
            ],
            'potion_heal_major' => [
                'slug' => 'potion-heal-major',
                'damage' => null,
                'element' => Element::None,
                'heal' => 15,
                'name' => 'Soin majeur',
                'description' => 'Une puissante potion qui restaure une grande quantité de points de vie',
                'hit' => 100,
                'level' => 1,
            ],
            'antidote_heal' => [
                'slug' => 'antidote-heal',
                'damage' => null,
                'element' => Element::None,
                'heal' => 3,
                'name' => 'Antidote',
                'description' => 'Un remède qui purifie le corps et soigne légèrement',
                'hit' => 100,
                'level' => 1,
            ],
            'bread_heal' => [
                'slug' => 'bread-heal',
                'damage' => null,
                'element' => Element::None,
                'heal' => 4,
                'name' => 'Pain nourrissant',
                'description' => 'Un morceau de pain qui redonne des forces',
                'hit' => 100,
                'level' => 1,
            ],
            'grilled_meat_heal' => [
                'slug' => 'grilled-meat-heal',
                'damage' => null,
                'element' => Element::None,
                'heal' => 8,
                'name' => 'Viande revigorante',
                'description' => 'Une viande grillée à point qui revigore le corps',
                'hit' => 100,
                'level' => 1,
            ],
            'stew_heal' => [
                'slug' => 'stew-heal',
                'damage' => null,
                'element' => Element::None,
                'heal' => 12,
                'name' => 'Ragoût reconstituant',
                'description' => 'Un copieux ragoût qui restaure profondément le corps et l\'esprit',
                'hit' => 100,
                'level' => 1,
            ],

            // --- Sorts consommables alchimiste ---
            'potion_heal_medium_spell' => [
                'slug' => 'potion-heal-medium',
                'damage' => null,
                'element' => Element::None,
                'heal' => 8,
                'name' => 'Soin modéré',
                'description' => 'Restaure une quantité modérée de points de vie',
                'hit' => 100,
                'level' => 1,
            ],
            'poison_vial_spell' => [
                'slug' => 'poison-vial',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Poison virulent',
                'description' => 'Un poison concentré qui ronge la cible',
                'hit' => 90,
                'level' => 1,
                'statusEffectSlug' => 'poison',
            ],
            'elixir_force_spell' => [
                'slug' => 'elixir-force',
                'damage' => null,
                'element' => Element::None,
                'heal' => 1,
                'name' => 'Rage alchimique',
                'description' => 'Décuple la force au prix du contrôle',
                'hit' => 100,
                'level' => 1,
                'statusEffectSlug' => 'berserk',
            ],
            'elixir_defense_spell' => [
                'slug' => 'elixir-defense',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 2,
                'name' => 'Bouclier alchimique',
                'description' => 'Un bouclier magique protège le buveur',
                'hit' => 100,
                'level' => 1,
                'statusEffectSlug' => 'shield',
            ],
            'onguent_healing_spell' => [
                'slug' => 'onguent-healing',
                'damage' => null,
                'element' => Element::None,
                'heal' => 4,
                'name' => 'Régénération douce',
                'description' => 'Un baume qui régénère les chairs lentement',
                'hit' => 100,
                'level' => 1,
                'statusEffectSlug' => 'regeneration',
            ],
            'elixir_vitality_spell' => [
                'slug' => 'elixir-vitality',
                'damage' => null,
                'element' => Element::None,
                'heal' => 18,
                'name' => 'Vitalité suprême',
                'description' => 'Restaure une quantité considérable de points de vie',
                'hit' => 100,
                'level' => 1,
            ],
            // --- Sort consommable charpentier (ECO-30) ---
            // La flèche est le **consommable perpétuel** du métier : elle se
            // dépense à chaque usage et ne se garde pas. C'est ce qui donne au
            // charpentier une demande qui ne s'éteint pas quand tout le monde a
            // son arc — le défaut de tout métier qui ne produit que du durable.
            'arrow_volley_spell' => [
                'slug' => 'arrow-volley',
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Volée de flèches',
                'description' => 'Une salve empennée qui trouve la faille de l\'armure',
                'hit' => 95,
                'level' => 1,
            ],
            // Sorts sans élément
            'none_attack_1' => [
                'slug' => 'none-attack-1',
                'damage' => 1,
                'element' => Element::None,
                'heal' => null,
                'name' => 'Attaque',
                'description' => 'Attaque 1',
                'hit' => 90,
                'level' => 1,
            ],
            'none_attack_2' => [
                'slug' => 'none-attack-2',
                'damage' => 2,
                'element' => Element::None,
                'heal' => null,
                'name' => 'Attaque',
                'description' => 'Attaque 2',
                'hit' => 90,
                'level' => 1,
            ],
            'none_attack_3' => [
                'slug' => 'none-attack-3',
                'damage' => 3,
                'element' => Element::None,
                'heal' => null,
                'name' => 'Attaque',
                'description' => 'Attaque 3',
                'hit' => 90,
                'level' => 1,
            ],
            'none_heal_2' => [
                'slug' => 'none-heal-2',
                'damage' => null,
                'element' => Element::None,
                'heal' => 2,
                'name' => 'Soin',
                'description' => 'Soin 2',
                'hit' => 90,
                'level' => 1,
            ],

            // === Materia Tier 2 — 1 par élément ===

            // Eau — Brume glaciale
            'frost_mist' => [
                'slug' => 'frost-mist',
                'damage' => 2,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Brume glaciale',
                'description' => 'Un brouillard de givre qui engourdit la cible',
                'hit' => 85,
                'energyCost' => 8,
                'statusEffectSlug' => 'paralysis',
                'level' => 2,
            ],
            // Air — Éclair en chaîne
            'air_chain_lightning' => [
                'slug' => 'air-chain-lightning',
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Éclair en chaîne',
                'description' => 'Des éclairs qui rebondissent entre les ennemis',
                'hit' => 85,
                'energyCost' => 10,
                'aoeTargets' => 0,
                'level' => 2,
            ],
            // Terre — Mur de pierre
            'stone_shield' => [
                'slug' => 'stone-shield',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 5,
                'name' => 'Mur de pierre',
                'description' => 'Érige un mur de pierre qui protège des attaques',
                'hit' => 100,
                'energyCost' => 10,
                'cooldown' => 3,
                'statusEffectSlug' => 'shield',
                'level' => 2,
            ],
            // Métal — Riposte d'acier
            'steel_riposte' => [
                'slug' => 'steel-riposte',
                'register' => CombatRegister::Melee,
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => 2,
                'name' => 'Riposte d\'acier',
                'description' => 'Une contre-attaque métallique qui blesse et protège',
                'hit' => 95,
                'energyCost' => 0,
                'cooldown' => 1,
                'level' => 2,
            ],
            // Bête — Morsure sauvage
            'savage_bite' => [
                'slug' => 'savage-bite',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Morsure sauvage',
                'description' => 'Une morsure féroce qui déchire la cible',
                'hit' => 90,
                'critical' => 12,
                'energyCost' => 7,
                'level' => 2,
            ],
            // Lumière — Bénédiction
            'light_blessing' => [
                'slug' => 'light-blessing',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 8,
                'name' => 'Bénédiction',
                'description' => 'Une bénédiction de lumière qui restaure la santé',
                'hit' => 100,
                'energyCost' => 10,
                'level' => 2,
            ],
            // Ombre — Drain vital
            'vital_drain' => [
                'slug' => 'vital-drain',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => 2,
                'name' => 'Drain vital',
                'description' => 'Aspire la force vitale de l\'ennemi',
                'hit' => 85,
                'energyCost' => 8,
                'level' => 2,
            ],

            // === Materia Tier 3 — 1 par élément (Épique) ===

            // Feu — Explosion solaire
            'solar_burst' => [
                'slug' => 'solar-burst',
                'damage' => 6,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Explosion solaire',
                'description' => 'Libère une explosion de chaleur solaire qui carbonise tout sur son passage',
                'hit' => 85,
                'energyCost' => 16,
                'cooldown' => 2,
                'statusEffectSlug' => 'burn',
                'critical' => 12,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            // Eau — Maelström glacial
            'frost_maelstrom' => [
                'slug' => 'frost-maelstrom',
                'damage' => 5,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Maelström glacial',
                'description' => 'Un tourbillon de glace qui aspire et gèle les ennemis',
                'hit' => 80,
                'energyCost' => 15,
                'cooldown' => 2,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 0,
                'level' => 3,
            ],
            // Air — Tempête de foudre
            'thunder_storm' => [
                'slug' => 'thunder-storm',
                'damage' => 6,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Tempête de foudre',
                'description' => 'Déchaîne une tempête d\'éclairs qui frappe aveuglément la zone',
                'hit' => 80,
                'energyCost' => 17,
                'cooldown' => 2,
                'critical' => 10,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            // Terre — Séisme cristallin
            'crystal_quake' => [
                'slug' => 'crystal-quake',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => 3,
                'name' => 'Séisme cristallin',
                'description' => 'Fait jaillir des cristaux du sol qui blessent et forment un bouclier protecteur',
                'hit' => 90,
                'energyCost' => 15,
                'cooldown' => 2,
                'statusEffectSlug' => 'shield',
                'level' => 3,
            ],
            // Métal — Lame d'orichalque
            'orichalcum_blade' => [
                'slug' => 'orichalcum-blade',
                'damage' => 7,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Lame d\'orichalque',
                'description' => 'Forge instantanément une lame d\'orichalque qui tranche les défenses les plus solides',
                'hit' => 90,
                'energyCost' => 14,
                'critical' => 18,
                'level' => 3,
            ],
            // Bête — Éveil primordial
            'primal_awakening' => [
                'slug' => 'primal-awakening',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => 3,
                'name' => 'Éveil primordial',
                'description' => 'Éveille les forces primordiales de la nature, empoisonnant les ennemis et revitalisant le lanceur',
                'hit' => 85,
                'energyCost' => 15,
                'statusEffectSlug' => 'poison',
                'level' => 3,
            ],
            // Lumière — Grâce divine
            'divine_grace' => [
                'slug' => 'divine-grace',
                'damage' => null,
                'element' => Element::Light,
                'heal' => 12,
                'name' => 'Grâce divine',
                'description' => 'Canalise la grâce des cieux pour une guérison profonde et durable',
                'hit' => 100,
                'energyCost' => 17,
                'cooldown' => 3,
                'statusEffectSlug' => 'regeneration-strong',
                'level' => 3,
            ],
            // Ombre — Pacte des ombres
            'shadow_covenant' => [
                'slug' => 'shadow-covenant',
                'damage' => 5,
                'element' => Element::Dark,
                'heal' => 3,
                'name' => 'Pacte des ombres',
                'description' => 'Scelle un pacte avec les ténèbres, drainant la vie de l\'ennemi tout en empoisonnant son âme',
                'hit' => 80,
                'energyCost' => 16,
                'cooldown' => 2,
                'statusEffectSlug' => 'poison-strong',
                'critical' => 10,
                'level' => 3,
            ],

            // === Sorts de boss de zone (tâche 66) ===
            // Gardien de la Forêt — Rugissement primordial (signature phase 2)
            'primordial_roar' => [
                'slug' => 'primordial-roar',
                'damage' => 7,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Rugissement primordial',
                'description' => 'Un rugissement ancestral qui fait trembler la terre et paralyse de terreur',
                'hit' => 80,
                'cooldown' => 3,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 0,
                'critical' => 10,
                'level' => 5,
            ],
            // Seigneur de la Forge — Forge obscure (signature phase 3)
            'dark_forge_blast' => [
                'slug' => 'dark-forge-blast',
                'damage' => 9,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Déflagration de la forge obscure',
                'description' => 'Libère une explosion de métal en fusion imprégné de ténèbres',
                'hit' => 75,
                'cooldown' => 4,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 0,
                'critical' => 15,
                'level' => 5,
            ],
            // === Sorts du boss final : Gardien de la Convergence (tache 94 — Acte 3) ===
            'convergence_pulse' => [
                'slug' => 'convergence-pulse',
                'damage' => 10,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Pulse de convergence',
                'description' => 'Le Gardien canalise l\'energie des quatre fragments et libere une onde devastatrice',
                'hit' => 85,
                'cooldown' => 3,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 0,
                'critical' => 15,
                'level' => 5,
            ],
            'amethyst_shatter' => [
                'slug' => 'amethyst-shatter',
                'damage' => 12,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Fracture d\'amethyste',
                'description' => 'Le cristal d\'amethyste explose en eclats mortels qui transpercent tout sur leur passage',
                'hit' => 80,
                'cooldown' => 4,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 0,
                'critical' => 20,
                'level' => 5,
            ],
            'fragment_barrier' => [
                'slug' => 'fragment-barrier',
                'damage' => 0,
                'element' => Element::Earth,
                'heal' => 5,
                'name' => 'Barriere des fragments',
                'description' => 'Les quatre fragments projettent un bouclier protecteur autour du Gardien',
                'hit' => 100,
                'cooldown' => 5,
                'statusEffectSlug' => 'shield-strong',
                'aoeTargets' => 0,
                'critical' => 0,
                'level' => 5,
            ],

            // === Sorts tier 2-3 : zone, soin avancé, contrôle (tâche 144) ===

            // Feu — Sort de zone
            'inferno_wave' => [
                'slug' => 'inferno-wave',
                'damage' => 5,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Vague infernale',
                'description' => 'Une déferlante de flammes qui embrase tout le champ de bataille',
                'hit' => 80,
                'energyCost' => 18,
                'cooldown' => 3,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // Feu — Soin défensif (rare pour le feu)
            'ember_ward' => [
                'slug' => 'ember-ward',
                'damage' => 0,
                'element' => Element::Fire,
                'heal' => 6,
                'name' => 'Garde de braise',
                'description' => 'Les braises forment un cocon protecteur qui cautérise les blessures',
                'hit' => 100,
                'energyCost' => 10,
                'statusEffectSlug' => 'shield',
                'level' => 3,
            ],

            // Eau — Contrôle (utilise freeze, jamais utilisé avant)
            'glacial_prison' => [
                'slug' => 'glacial-prison',
                'damage' => 4,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Prison glaciale',
                'description' => 'Enferme l\'ennemi dans un bloc de glace qui ralentit ses mouvements',
                'hit' => 85,
                'energyCost' => 10,
                'statusEffectSlug' => 'freeze',
                'level' => 3,
            ],
            // Eau — Soin avancé avec régénération
            'purifying_spring' => [
                'slug' => 'purifying-spring',
                'damage' => null,
                'element' => Element::Water,
                'heal' => 8,
                'name' => 'Source purificatrice',
                'description' => 'Invoque une source d\'eau pure dont les vertus régénèrent le corps en profondeur',
                'hit' => 100,
                'energyCost' => 12,
                'statusEffectSlug' => 'regeneration',
                'level' => 3,
            ],

            // Air — Sort de zone + contrôle
            'tempest_fury' => [
                'slug' => 'tempest-fury',
                'damage' => 5,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Fureur de la tempête',
                'description' => 'Déchaîne une tempête violente qui foudroie et paralyse les ennemis',
                'hit' => 80,
                'energyCost' => 16,
                'cooldown' => 3,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // Air — Soin
            'breath_of_wind' => [
                'slug' => 'breath-of-wind',
                'damage' => null,
                'element' => Element::Air,
                'heal' => 7,
                'name' => 'Souffle du vent',
                'description' => 'Une brise vivifiante qui apaise les douleurs et revigore le corps',
                'hit' => 100,
                'energyCost' => 8,
                'level' => 2,
            ],

            // Terre — Sort de zone
            'fissure' => [
                'slug' => 'fissure',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Fissure',
                'description' => 'Le sol se fissure violemment, libérant des vapeurs toxiques des profondeurs',
                'hit' => 85,
                'energyCost' => 12,
                'statusEffectSlug' => 'poison',
                'aoeTargets' => 0,
                'level' => 3,
            ],
            // Terre — Soin défensif avancé
            // --- Les quatre gestes du Gardien (ARC-08d) ----------------------
            //
            // Le canon (§ 14.2) en annoncait **deux** a creer — Racines et Seve
            // de pierre. Il en fallait quatre : le Bouclier terreux et le
            // Bouclier magique sont **partages avec le Geomancien**, donc de
            // registre `spell`, donc hors de portee d'un arbre de melee. C'est
            // la meme regle qu'ARC-08b et ARC-08c ont rencontree — *un accord
            // partage garde le registre de celui qui l'a ouvert le premier* —,
            // et c'est la troisieme fois qu'elle decide du contenu a ecrire.
            //
            // **Les trois gestes de groupe sont des depots** (§ 7 bis), et pour
            // un archetype d'entretien ce n'est pas une contrainte mais sa
            // definition : *il ne soigne pas, il provisionne*.
            'earthen_guard' => [
                'slug' => 'earthen-guard',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 4,
                'name' => 'Bouclier terreux',
                'description' => 'La terre qu\'on ramene sur soi, et qui tient le premier coup',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 0,
                'statusEffectSlug' => 'shield',
                'level' => 1,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::SelfOnly,
            ],
            'roots' => [
                'slug' => 'roots',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 5,
                'name' => 'Racines',
                'description' => 'Ce qu\'on plante a cote de quelqu\'un continue de pousser',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 1,
                'statusEffectSlug' => 'regeneration',
                'level' => 2,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Heal,
                'scope' => SpellScope::Ally,
            ],
            'stone_bulwark' => [
                'slug' => 'stone-bulwark',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 3,
                'name' => 'Bouclier magique',
                'description' => 'La pierre se leve autour des siens, pas autour de soi',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 2,
                'statusEffectSlug' => 'shield',
                'level' => 3,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::Group,
            ],
            'stone_sap' => [
                'slug' => 'stone-sap',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 6,
                'name' => 'Seve de pierre',
                'description' => 'Ce qui coule dans la roche coule longtemps, et pour tout le monde',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 3,
                'statusEffectSlug' => 'regeneration',
                'level' => 4,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Heal,
                'scope' => SpellScope::Group,
            ],
            'stonewall' => [
                'slug' => 'stonewall',
                'damage' => null,
                'element' => Element::Earth,
                'heal' => 4,
                'name' => 'Rempart de pierre',
                'description' => 'Un mur qu\'on dresse pour les autres, et qui tient sans nous',
                'hit' => 100,
                'energyCost' => 0,
                'cooldown' => 3,
                'statusEffectSlug' => 'shield-strong',
                'level' => 4,
                'register' => CombatRegister::Melee,
                'intent' => SpellIntent::Protection,
                'scope' => SpellScope::Group,
            ],

            // Métal — Sort de zone
            'blade_storm' => [
                'slug' => 'blade-storm',
                'damage' => 5,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Tempête de lames',
                'description' => 'Des dizaines de lames métalliques tourbillonnent et lacèrent tout dans la zone',
                'hit' => 80,
                'energyCost' => 15,
                'cooldown' => 2,
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // Métal — Contrôle (utilise silence, jamais utilisé avant)
            'metal_silence' => [
                'slug' => 'metal-silence',
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Résonance métallique',
                'description' => 'Un son strident émis par le métal qui étourdit et réduit au silence l\'ennemi',
                'hit' => 85,
                'energyCost' => 10,
                'statusEffectSlug' => 'silence',
                'level' => 3,
            ],

            // Bête — Sort de zone
            'stampede' => [
                'slug' => 'stampede',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Charge sauvage',
                'description' => 'Invoque un troupeau spectral qui piétine tout sur son passage',
                'hit' => 85,
                'energyCost' => 12,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            // Bête — Soin avancé puissant
            'wild_regeneration' => [
                'slug' => 'wild-regeneration',
                'damage' => null,
                'element' => Element::Beast,
                'heal' => 10,
                'name' => 'Régénération sauvage',
                'description' => 'La force vitale de la nature restaure profondément le corps et accélère la guérison',
                'hit' => 100,
                'energyCost' => 16,
                'cooldown' => 3,
                'statusEffectSlug' => 'regeneration-strong',
                'level' => 4,
            ],

            // Lumière — Sort de zone hybride (dégâts + soin)
            'holy_nova' => [
                'slug' => 'holy-nova',
                'damage' => 3,
                'element' => Element::Light,
                'heal' => 5,
                'name' => 'Nova sacrée',
                'description' => 'Une explosion de lumière pure qui blesse les ennemis tout en soignant le lanceur',
                'hit' => 90,
                'energyCost' => 18,
                'cooldown' => 3,
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // Lumière — Contrôle (silence)
            'purge' => [
                'slug' => 'purge',
                'damage' => 2,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Purge lumineuse',
                'description' => 'Un éclat de lumière aveuglant qui coupe l\'ennemi de ses pouvoirs magiques',
                'hit' => 90,
                'energyCost' => 10,
                'statusEffectSlug' => 'silence',
                'level' => 3,
            ],

            // Ombre — Sort de zone
            'void_eruption' => [
                'slug' => 'void-eruption',
                'damage' => 6,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Éruption du vide',
                'description' => 'Le néant explose en une onde de ténèbres qui empoisonne et consume les ennemis',
                'hit' => 80,
                'energyCost' => 18,
                'cooldown' => 3,
                'statusEffectSlug' => 'poison-strong',
                'aoeTargets' => 0,
                'level' => 4,
            ],
            // Ombre — Contrôle (drain + berserk, jamais utilisé avant)
            'abyssal_fury' => [
                'slug' => 'abyssal-fury',
                'damage' => 5,
                'element' => Element::Dark,
                'heal' => 3,
                'name' => 'Fureur abyssale',
                'description' => 'Canalise la rage des abysses : blesse l\'ennemi et déchaîne la fureur intérieure du lanceur',
                'hit' => 85,
                'energyCost' => 14,
                'cooldown' => 3,
                'statusEffectSlug' => 'berserk',
                'level' => 4,
            ],
            // === Sorts tier 2-3 : couverture 8 éléments + équilibrage synergies (tâche 144) ===

            // Feu — 3 sorts (synergies : Eau→Vapeur, Terre→Florale, Métal→Forge)
            'ember_lance' => [
                'slug' => 'ember-lance',
                'damage' => 3,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Lance de braise',
                'description' => 'Un trait de braise perçant qui enflamme la cible au contact',
                'hit' => 90,
                'energyCost' => 6,
                'statusEffectSlug' => 'burn',
                'critical' => 10,
                'level' => 2,
            ],
            'scorching_vortex' => [
                'slug' => 'scorching-vortex',
                'damage' => 5,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Vortex ardent',
                'description' => 'Un tourbillon de flammes qui consume tout dans la zone',
                'hit' => 85,
                'energyCost' => 14,
                'cooldown' => 2,
                'aoeTargets' => 0,
                'critical' => 8,
                'level' => 3,
            ],
            'pyroclasm' => [
                'slug' => 'pyroclasm',
                'damage' => 7,
                'element' => Element::Fire,
                'heal' => null,
                'name' => 'Pyroclasme',
                'description' => 'Une éruption volcanique dévastatrice qui calcine le champ de bataille',
                'hit' => 80,
                'energyCost' => 20,
                'cooldown' => 3,
                'statusEffectSlug' => 'burn',
                'aoeTargets' => 0,
                'critical' => 12,
                'level' => 4,
            ],

            // Eau — 3 sorts (synergie : Feu→Vapeur)
            'ice_shard' => [
                'slug' => 'ice-shard',
                'damage' => 3,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Éclat de glace',
                'description' => 'Un fragment de glace acéré qui gèle la cible au contact',
                'hit' => 90,
                'energyCost' => 5,
                'statusEffectSlug' => 'freeze',
                'critical' => 8,
                'level' => 2,
            ],
            'torrent_strike' => [
                'slug' => 'torrent-strike',
                'damage' => 4,
                'element' => Element::Water,
                'heal' => null,
                'name' => 'Frappe torrentielle',
                'description' => 'Un déluge violent qui submerge et paralyse les ennemis',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'abyssal_tide' => [
                'slug' => 'abyssal-tide',
                'damage' => 6,
                'element' => Element::Water,
                'heal' => 2,
                'name' => 'Marée abyssale',
                'description' => 'Les eaux des profondeurs déferlent, noyant les ennemis et régénérant le lanceur',
                'hit' => 80,
                'energyCost' => 18,
                'cooldown' => 3,
                'aoeTargets' => 0,
                'level' => 4,
            ],

            // Air — 3 sorts (synergie : Terre→Tempête de sable)
            'razor_gust' => [
                'slug' => 'razor-gust',
                'damage' => 3,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Rafale tranchante',
                'description' => 'Une bourrasque coupante comme une lame qui lacère l\'ennemi',
                'hit' => 92,
                'energyCost' => 5,
                'critical' => 10,
                'level' => 2,
            ],
            'vortex_snare' => [
                'slug' => 'vortex-snare',
                'damage' => 4,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Piège tourbillonnant',
                'description' => 'Un vortex qui emprisonne et paralyse les ennemis dans son souffle',
                'hit' => 85,
                'energyCost' => 10,
                'statusEffectSlug' => 'paralysis',
                'level' => 3,
            ],
            'stratosphere_burst' => [
                'slug' => 'stratosphere-burst',
                'damage' => 6,
                'element' => Element::Air,
                'heal' => null,
                'name' => 'Explosion stratosphérique',
                'description' => 'L\'air comprimé explose avec une force dévastatrice sur toute la zone',
                'hit' => 80,
                'energyCost' => 16,
                'cooldown' => 3,
                'aoeTargets' => 0,
                'critical' => 10,
                'level' => 4,
            ],

            // Terre — 3 sorts (synergies : Air→Tempête, Feu→Florale, Bête→Furie)
            'gravel_shot' => [
                'slug' => 'gravel-shot',
                'damage' => 3,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Salve de gravier',
                'description' => 'Une volée de pierres tranchantes projetées à grande vitesse',
                'hit' => 90,
                'energyCost' => 5,
                'critical' => 8,
                'level' => 2,
            ],
            'tremor_wave' => [
                'slug' => 'tremor-wave',
                'damage' => 4,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Onde sismique',
                'description' => 'Le sol tremble violemment, déstabilisant tous les ennemis de la zone',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'obsidian_lance' => [
                'slug' => 'obsidian-lance',
                'damage' => 6,
                'element' => Element::Earth,
                'heal' => null,
                'name' => 'Lance d\'obsidienne',
                'description' => 'Une lance de roche volcanique qui transperce les défenses les plus solides',
                'hit' => 80,
                'energyCost' => 16,
                'cooldown' => 3,
                'critical' => 15,
                'level' => 4,
            ],

            // Métal — 3 sorts (synergies : Feu→Forge, Lumière→Lame sacrée)
            'steel_barrage' => [
                'slug' => 'steel-barrage',
                'register' => CombatRegister::Melee,
                'damage' => 3,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Barrage d\'acier',
                'description' => 'Une rafale de projectiles métalliques à haute vélocité',
                'hit' => 90,
                'energyCost' => 0,
                'critical' => 12,
                'cooldown' => 1,
                'level' => 2,
            ],
            'shrapnel_blast' => [
                'slug' => 'shrapnel-blast',
                'damage' => 4,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Explosion de shrapnel',
                'description' => 'Des éclats métalliques explosent dans toutes les directions',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'titanium_crush' => [
                'slug' => 'titanium-crush',
                'register' => CombatRegister::Melee,
                'damage' => 7,
                'element' => Element::Metal,
                'heal' => null,
                'name' => 'Écrasement de titane',
                'description' => 'Un poing de titane s\'abat avec une force colossale sur la cible',
                'hit' => 80,
                'energyCost' => 0,
                'cooldown' => 3,
                'critical' => 15,
                'level' => 4,
            ],

            // Bête — 3 sorts (synergies : Terre→Furie primale, Ombre→Ombre venimeuse)
            'claw_swipe' => [
                'slug' => 'claw-swipe',
                'damage' => 3,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Coup de griffe',
                'description' => 'Un coup de griffes sauvage qui empoisonne la cible',
                'hit' => 90,
                'energyCost' => 5,
                'statusEffectSlug' => 'poison',
                'level' => 2,
            ],
            'pack_assault' => [
                'slug' => 'pack-assault',
                'damage' => 4,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Assaut de meute',
                'description' => 'Invoque des esprits animaux qui assaillent tous les ennemis',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'apex_predator' => [
                'slug' => 'apex-predator',
                'damage' => 7,
                'element' => Element::Beast,
                'heal' => null,
                'name' => 'Prédateur suprême',
                'description' => 'Canalise l\'instinct du prédateur ultime pour un assaut dévastateur',
                'hit' => 80,
                'energyCost' => 18,
                'cooldown' => 3,
                'statusEffectSlug' => 'berserk',
                'level' => 4,
            ],

            // Lumière — 3 sorts (synergies : Ombre→Éclipse, Métal→Lame sacrée)
            'luminous_arrow' => [
                'slug' => 'luminous-arrow',
                'damage' => 3,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Flèche lumineuse',
                'description' => 'Un trait de lumière pure qui transperce les ténèbres',
                'hit' => 95,
                'energyCost' => 5,
                'critical' => 8,
                'level' => 2,
            ],
            'solar_flare' => [
                'slug' => 'solar-flare',
                'damage' => 4,
                'element' => Element::Light,
                'heal' => null,
                'name' => 'Éruption solaire',
                'description' => 'Un éclat solaire aveuglant qui brûle et réduit au silence les ennemis',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'statusEffectSlug' => 'silence',
                'aoeTargets' => 0,
                'level' => 3,
            ],
            'celestial_judgment' => [
                'slug' => 'celestial-judgment',
                'damage' => 6,
                'element' => Element::Light,
                'heal' => 3,
                'name' => 'Jugement céleste',
                'description' => 'La lumière divine frappe les impurs et restaure le lanceur',
                'hit' => 80,
                'energyCost' => 18,
                'cooldown' => 3,
                'aoeTargets' => 0,
                'level' => 4,
            ],

            // Ombre — 3 sorts (synergies : Lumière→Éclipse, Bête→Ombre venimeuse)
            'shadow_claw' => [
                'slug' => 'shadow-claw',
                'damage' => 3,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Griffe d\'ombre',
                'description' => 'Une griffe de ténèbres qui empoisonne l\'âme de la cible',
                'hit' => 90,
                'energyCost' => 5,
                'statusEffectSlug' => 'poison',
                'level' => 2,
            ],
            // ARC-08a — l'entrave de zone du Necromancien (§ 12.2). Sa
            // description promettait « les ennemis » depuis toujours ; son
            // `aoeTargets` valait **0**, donc le geste ne touchait qu'une cible
            // et sa portee se derivait en `Target`. Les deux se rejoignent : la
            // portee n'est pas declaree ici, elle **suit** l'aire, sans quoi
            // l'etiquette dirait « plusieurs cibles » d'un geste qui n'en
            // frappe qu'une.
            'nightmare_pulse' => [
                'slug' => 'nightmare-pulse',
                'damage' => 4,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Pulsation cauchemardesque',
                'description' => 'Une onde de terreur pure qui paralyse les ennemis de peur',
                'hit' => 85,
                'energyCost' => 12,
                'cooldown' => 2,
                'statusEffectSlug' => 'paralysis',
                'aoeTargets' => 2,
                'level' => 3,
                'intent' => SpellIntent::Hinder,
            ],
            'void_collapse' => [
                'slug' => 'void-collapse',
                'damage' => 7,
                'element' => Element::Dark,
                'heal' => null,
                'name' => 'Effondrement du vide',
                'description' => 'Le néant s\'effondre sur lui-même, engloutissant tout dans les ténèbres',
                'hit' => 80,
                'energyCost' => 20,
                'cooldown' => 3,
                'statusEffectSlug' => 'poison-strong',
                'aoeTargets' => 0,
                'critical' => 10,
                'level' => 4,
            ],
        ];
    }
}
